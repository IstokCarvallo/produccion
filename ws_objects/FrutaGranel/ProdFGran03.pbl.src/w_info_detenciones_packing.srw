$PBExportHeader$w_info_detenciones_packing.srw
$PBExportComments$Informe Detenciones Packing
forward
global type w_info_detenciones_packing from w_para_informes
end type
type dw_1 from datawindow within w_info_detenciones_packing
end type
end forward
...
integer width = 2395
integer height = 1224
string title = "Informe Detenciones Packing"
dw_1 dw_1
end type
global w_info_detenciones_packing w_info_detenciones_packing

type variables


DataWindowChild	idwc_planta, idwc_especie, idwc_linea, idwc_area,idwc_exportadores
...
on w_info_detenciones_packing.create
int iCurrent
call super::create
...
on w_info_detenciones_packing.destroy
call super::destroy
destroy(this.dw_1)
end on
...
type pb_excel from w_para_informes`pb_excel within w_info_detenciones_packing
end type

type st_computador from w_para_informes`st_computador within w_info_detenciones_packing
integer x = 1399
end type

type st_usuario from w_para_informes`st_usuario within w_info_detenciones_packing
integer x = 1399
end type

type st_temporada from w_para_informes`st_temporada within w_info_detenciones_packing
integer x = 1399
end type

type p_logo from w_para_informes`p_logo within w_info_detenciones_packing
end type

type st_titulo from w_para_informes`st_titulo within w_info_detenciones_packing
integer x = 160
integer width = 1499
string text = "Informe General Detenciones Packing"
end type

type pb_acepta from w_para_informes`pb_acepta within w_info_detenciones_packing
integer x = 1943
integer y = 312
integer taborder = 60
end type
...
istr_info.titulo	= 'INFORME DETENCIONES PACKING'

OpenWithParm(vinf, istr_info)

vinf.dw_1.DataObject = "dw_info_detencionlinea_packing_enc"
dw_1.accepttext()
...
IF IsNull( li_area) THEN
	MessageBox( "Area Responsable Errónea", "Falta seleccionar un Area Responsable.", StopSign!, Ok!)
	RETURN 1				 
END If
...
ll_Fila	=	vinf.dw_1.Retrieve( li_Area,li_planta,li_Linea,li_Especie, ld_FechaCorte, li_Turno)
...
type pb_salir from w_para_informes`pb_salir within w_info_detenciones_packing
integer x = 1943
integer y = 588
integer taborder = 70
end type
...
string title = "none"
string dataobject = "dw_info_detenciones_lineas_seleccion"
