$PBExportHeader$w_info_control_calidad.srw
$PBExportComments$Ventana de Informe de Control de Calidad Recepción Distribución Calibres y Color
forward
global type w_info_control_calidad from w_para_informes
end type
type dw_1 from datawindow within w_info_control_calidad
end type
type rb_detprod from radiobutton within w_info_control_calidad
end type
type rb_resprod from radiobutton within w_info_control_calidad
end type
type rb_detfrigo from radiobutton within w_info_control_calidad
end type
type rb_resfrigo from radiobutton within w_info_control_calidad
end type
type gb_3 from groupbox within w_info_control_calidad
end type
end forward
...
rb_resprod rb_resprod
rb_detfrigo rb_detfrigo
rb_resfrigo rb_resfrigo
gb_3 gb_3
end type
global w_info_control_calidad w_info_control_calidad
...
on w_info_control_calidad.create
int iCurrent
call super::create
this.dw_1=create dw_1
this.rb_detprod=create rb_detprod
this.rb_resprod=create rb_resprod
this.rb_detfrigo=create rb_detfrigo
this.rb_resfrigo=create rb_resfrigo
this.gb_3=create gb_3
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.dw_1
this.Control[iCurrent+2]=this.rb_detprod
this.Control[iCurrent+3]=this.rb_resprod
this.Control[iCurrent+4]=this.rb_detfrigo
this.Control[iCurrent+5]=this.rb_resfrigo
this.Control[iCurrent+6]=this.gb_3
end on

on w_info_control_calidad.destroy
call super::destroy
destroy(this.dw_1)
destroy(this.rb_detprod)
destroy(this.rb_resprod)
destroy(this.rb_detfrigo)
destroy(this.rb_resfrigo)
destroy(this.gb_3)
end on
...
type st_titulo from w_para_informes`st_titulo within w_info_control_calidad
...
type pb_acepta from w_para_informes`pb_acepta within w_info_control_calidad
integer x = 2245
integer y = 268
integer height = 140
...
long	   ll_Planta, ll_Especie, ll_Variedad, ll_Periodo,  ll_Categoria, &
			ll_tipoenvase, ll_Fila, ll_camara, ll_productor, ll_Informe, ll_exporta
...
vinf.dw_1.DataObject = "dw_info_detalle_control_calidad"
dw_1.accepttext()

//acepta Exportador
ll_exporta = dw_1.Object.exportador[1]
IF IsNull(ll_exporta) Then
...
// Tipo de Informe
IF rb_detprod.Checked THEN
	ll_Informe = 1
ELSEIF rb_detfrigo.Checked THEN
		 ll_Informe = 2
	 ELSEIF rb_resprod.Checked THEN
		     ll_Informe = 3
		 ELSE 
			  ll_Informe = 4
END IF
...
ll_Fila	=	vinf.dw_1.Retrieve(ll_Planta,ll_Camara, ll_Especie, ll_Variedad,  ll_Productor, &
                               ls_Tratamiento, ld_fechadesde, ld_fechahasta, ll_Informe, ll_exporta)
...
ELSE
	...
	IF ll_Informe = 1 THEN vinf.dw_1.Modify("Datawindow.Header.2.Height=0")
	IF ll_Informe = 1 THEN vinf.dw_1.Modify("Datawindow.Trailer.2.Height=0")
	IF ll_Informe = 1 THEN vinf.dw_1.Modify("Datawindow.Trailer.4.Height=0")	

	IF ll_Informe = 2 THEN vinf.dw_1.Modify("Datawindow.Trailer.4.Height=0")		
	
	IF ll_Informe = 3 THEN vinf.dw_1.Modify("Datawindow.Header.2.Height=0")
	IF ll_Informe = 3 THEN vinf.dw_1.Modify("Datawindow.Header.4.Height=0")	
	IF ll_Informe = 3 THEN vinf.dw_1.Modify("Datawindow.Detail.Height=0")
	IF ll_Informe = 3 THEN vinf.dw_1.Modify("Datawindow.Trailer.2.Height=0")		


	IF ll_Informe = 4 THEN vinf.dw_1.Modify("Datawindow.Header.3.Height=0")
	IF ll_Informe = 4 THEN vinf.dw_1.Modify("Datawindow.Header.4.Height=0")	
	IF ll_Informe = 4 THEN vinf.dw_1.Modify("DataWindow.Trailer.3.Height=0")
	IF ll_Informe = 4 THEN vinf.dw_1.Modify("DataWindow.Detail.Height=0")
...
type pb_salir from w_para_informes`pb_salir within w_info_control_calidad
integer x = 2245
integer y = 556
integer height = 140
...
long textcolor = 33554432
long backcolor = 12632256
string text = "Detalle Frigorifico"
end type
...
long textcolor = 33554432
long backcolor = 12632256
string text = "Resumen Frigorifico"
end type
