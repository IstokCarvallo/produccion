$PBExportHeader$w_gene_cierreweb_orden_proceso.srw
$PBExportComments$Genera Datos para Pronóstico de Cierre
forward
global type w_gene_cierreweb_orden_proceso from window
end type
type uo_selcliente from uo_seleccion_clientesprod within w_gene_cierreweb_orden_proceso
end type
type uo_selplanta from uo_seleccion_plantas within w_gene_cierreweb_orden_proceso
end type
type dw_3 from uo_dw within w_gene_cierreweb_orden_proceso
end type
type dw_1 from uo_dw within w_gene_cierreweb_orden_proceso
end type
type st_12 from statictext within w_gene_cierreweb_orden_proceso
end type
type st_9 from statictext within w_gene_cierreweb_orden_proceso
end type
type sle_prod from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type sle_codpro from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type st_20 from statictext within w_gene_cierreweb_orden_proceso
end type
type sle_varie from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type sle_codvar from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type sle_espe from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type sle_codesp from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type cb_1 from commandbutton within w_gene_cierreweb_orden_proceso
end type
type em_fecha from editmask within w_gene_cierreweb_orden_proceso
end type
type st_8 from statictext within w_gene_cierreweb_orden_proceso
end type
type em_proceso from editmask within w_gene_cierreweb_orden_proceso
end type
type st_7 from statictext within w_gene_cierreweb_orden_proceso
end type
type ddlb_tipoproc from dropdownlistbox within w_gene_cierreweb_orden_proceso
end type
type st_6 from statictext within w_gene_cierreweb_orden_proceso
end type
type sle_mensa from singlelineedit within w_gene_cierreweb_orden_proceso
end type
type st_5 from statictext within w_gene_cierreweb_orden_proceso
end type
type pb_salir from picturebutton within w_gene_cierreweb_orden_proceso
end type
type pb_acepta from picturebutton within w_gene_cierreweb_orden_proceso
end type
type st_4 from statictext within w_gene_cierreweb_orden_proceso
end type
type st_2 from statictext within w_gene_cierreweb_orden_proceso
end type
type st_1 from statictext within w_gene_cierreweb_orden_proceso
end type
type st_titulo from statictext within w_gene_cierreweb_orden_proceso
end type
type r_1 from rectangle within w_gene_cierreweb_orden_proceso
end type
type st_3 from statictext within w_gene_cierreweb_orden_proceso
end type
end forward
...
global w_gene_cierreweb_orden_proceso w_gene_cierreweb_orden_proceso
...
uo_orderprocesopdf				iuo_GrabaPdf
...
public function boolean wf_genera_pdf ()
public function boolean wf_graba (datawindow adw, boolean borrando)
public function boolean wf_carga_tabla (integer ai_cliente, integer ai_planta, integer ai_tipo, long al_numero, string as_archivo, long al_productor)
end prototypes

event ue_validapassword();str_mant	lstr_mant

lstr_mant.Argumento[1]	=	"Granel - Cierre WEB de Proceso"
lstr_mant.Argumento[2]	=	gstr_paramplanta.pass_cierreweb
...
public function boolean wf_genera_pdf ();Boolean	lb_Retorno = True
String		ls_Archivo
Long		ll_Numero, ll_Productor, ll_Fila


ll_numero = long(em_proceso.text)
IF isnull(ll_numero) or ll_numero =0 THEN
	MessageBox("Atención","Debe Seleccionar un Numero de Proceso.",Exclamation!, OK!)
	lb_Retorno = False
End If


ll_Productor = long(sle_codpro.text)
IF IsNull(ll_Productor) or ll_Productor =0 THEN
	MessageBox("Atención","Debe Seleccionar un Numero de Proceso.",Exclamation!, OK!)
	lb_Retorno = False
End If

ls_archivo	=	'C:\' + String(uo_SelCliente.Codigo, '000') + String (uo_SelPlanta.Codigo, '0000') + String (ii_tiponum) + String(ll_Numero, '00000000') + '.PDF'

wf_carga_tabla(uo_SelCliente.Codigo, uo_SelPlanta.Codigo, ii_tiponum, ll_Numero, ls_archivo, ll_Productor)

If dw_1.Retrieve(uo_SelCliente.Codigo, uo_SelPlanta.Codigo, ll_numero, ii_tiponum, Integer(sle_codpro.Text), Integer(sle_codesp.Text), Integer(sle_codvar.Text), &
				-1, -1, Date(em_fecha.Text), Date(em_fecha.Text), -1, 1, 1, 2) = -1 Then
	MessageBox('Error', 'No se pudo generar información.')
	lb_Retorno = False
Else
	If dw_1.SaveAs(ls_Archivo, PDF!, False) = -1 Then
		MessageBox('Error', 'No se pudo generar Archivo: ' + ls_Archivo)
		lb_Retorno = False
	Else
		iuo_GrabaPdf.GrabaImagen(dw_3, dw_3.RowCount(), sqlca, ls_Archivo)
		If Not FileDelete(ls_Archivo)  Then MessageBox('Alerta', 'No se pudo eliminar el archivo: ' + ls_Archivo, Information!, OK!)
	End If
End If
...
public function boolean wf_carga_tabla (integer ai_cliente, integer ai_planta, integer ai_tipo, long al_numero, string as_archivo, long al_productor);Boolean	lb_Retorno = True
Integer	li_Secuencia 
Long		ll_New
String		ls_Path, ls_File

dw_3.SetTransObject(sqlca)
If dw_3.Retrieve(ai_cliente, ai_planta, ai_tipo, al_numero) = 0 Then
	li_Secuencia = 1 
Else
	li_Secuencia = dw_3.Object.Maximo[1] + 1
End If

ll_New = dw_3.InsertRow(0)

ls_Path	= Mid(as_Archivo, 1, LastPos(as_Archivo, '\'))
ls_File	= Mid(as_Archivo, LastPos(as_Archivo, '\') + 1, Len(as_Archivo))

dw_3.Object.clie_codigo[ll_New]	= ai_cliente
dw_3.Object.plde_codigo[ll_New]	= ai_planta
dw_3.Object.orpr_tipord[ll_New]	= ai_tipo
dw_3.Object.orpr_numero[ll_New]	= al_numero
dw_3.Object.orpr_secuen[ll_New]	= li_secuencia
dw_3.Object.orpr_fecpro[ll_New]	= Today()
dw_3.Object.orpr_rutas[ll_New]	= ls_Path
dw_3.Object.orpr_archiv[ll_New]	= ls_File
dw_3.Object.prod_codigo[ll_New]	= al_Productor

If Not wf_Graba(dw_3, False) Then lb_Retorno = True

Return lb_Retorno
end function

on w_gene_cierreweb_orden_proceso.create
...
on w_gene_cierreweb_orden_proceso.destroy
...
Else
	...
	iuo_GrabaPdf	=	Create uo_orderprocesopdf
...
integer y = 1420
integer taborder = 80
string dataobject = "dw_mant_mues_ordenprocesopdf"
...
integer y = 1024
integer taborder = 70
string dataobject = "dw_info_procesokilos_final"
boolean vscrollbar = false
end type
...
IF sle_mensa.text	= "Cierre Web" THEN
	MessageBox("Atención","La Orden de Proceso ya se encuentra con Cierre Web",Exclamation!)
	Return 1
END IF
...
ELSE
	UPDATE dbo.spro_movtofrutacomenca
	 ...
	 WHERE plde_codigo = :uo_SelPlanta.Codigo AND 
			 tpmv_codigo = 21 AND
			 mfco_tipdoc = :ii_tiponum AND  
			 mfco_docrel = :ll_numero AND
			 clie_codigo = :uo_SelCliente.Codigo;
...
Update dbo.spro_ordenproceso SET orpr_estado = 5
	WHERE plde_codigo	=	:uo_SelPlanta.Codigo
...
long backcolor = 16711680
boolean enabled = false
string text = "Cierre de Orden de Proceso"
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
