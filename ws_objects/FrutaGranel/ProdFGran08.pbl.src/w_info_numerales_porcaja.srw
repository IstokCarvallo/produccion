...
type sle_variedad from singlelineedit within w_info_numerales_porcaja
end type
type cb_buscavariedad from commandbutton within w_info_numerales_porcaja
end type
type em_variedad from editmask within w_info_numerales_porcaja
end type
...
type dw_especie from datawindow within w_info_numerales_porcaja
end type
...
type cbx_variedad from checkbox within w_info_numerales_porcaja
end type
type st_7 from statictext within w_info_numerales_porcaja
end type
type st_12 from statictext within w_info_numerales_porcaja
end type
type rb_bin from radiobutton within w_info_numerales_porcaja
end type
type rb_kilos from radiobutton within w_info_numerales_porcaja
end type
type gb_3 from groupbox within w_info_numerales_porcaja
end type
end forward
...
DataWindowChild 	idwc_planta, idwc_tipoen, idwc_envases, idwc_cliente
...
this.rb_bin=create rb_bin
this.rb_kilos=create rb_kilos
this.gb_3=create gb_3
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.gb_7
this.Control[iCurrent+2]=this.gb_4
this.Control[iCurrent+3]=this.st_2
this.Control[iCurrent+4]=this.sle_variedad
this.Control[iCurrent+5]=this.cb_buscavariedad
this.Control[iCurrent+6]=this.em_variedad
this.Control[iCurrent+7]=this.st_variedad
this.Control[iCurrent+8]=this.st_4
this.Control[iCurrent+9]=this.st_8
this.Control[iCurrent+10]=this.st_10
this.Control[iCurrent+11]=this.dw_cliente
this.Control[iCurrent+12]=this.cbx_planta
this.Control[iCurrent+13]=this.dw_planta
this.Control[iCurrent+14]=this.dw_tipoprod
this.Control[iCurrent+15]=this.dw_productor
this.Control[iCurrent+16]=this.dw_especie
this.Control[iCurrent+17]=this.st_5
this.Control[iCurrent+18]=this.dw_periodo
this.Control[iCurrent+19]=this.dw_envases
this.Control[iCurrent+20]=this.st_9
this.Control[iCurrent+21]=this.dw_tipoenvase
this.Control[iCurrent+22]=this.st_11
this.Control[iCurrent+23]=this.cbx_tipoenvases
this.Control[iCurrent+24]=this.st_3
this.Control[iCurrent+25]=this.st_6
this.Control[iCurrent+26]=this.st_1
this.Control[iCurrent+27]=this.cbx_productor
this.Control[iCurrent+28]=this.cbx_variedad
this.Control[iCurrent+29]=this.st_7
this.Control[iCurrent+30]=this.st_12
this.Control[iCurrent+31]=this.rb_bin
this.Control[iCurrent+32]=this.rb_kilos
...
IF isnull(idwc_cliente.Retrieve()) THEN
	dw_cliente.InsertRow(0)
END IF	

IF isnull(idwc_TipoProd.Retrieve()) THEN
	dw_tipoprod.InsertRow(0)
END IF	

IF isnull(idwc_productor.Retrieve(-1)) THEN
	dw_productor.InsertRow(0)
END IF
	
IF isnull(idwc_especie.Retrieve()) THEN
	dw_especie.InsertRow(0)
END IF

IF isnull(idwc_periodo.Retrieve(gi_CodExport)) THEN
...
IF idwc_Periodo.RowCount() > 0 THEN
...
//dw_periodo.Object.fape_numero[1]		=	ii_Periodo
dw_tipoprod.Object.tipr_codigo[1] 	= 	1
dw_productor.Object.prod_codigo[1]	=	gi_CodProductor
dw_especie.Object.espe_codigo[1]		=	gstr_paramplanta.codigoespecie
dw_envases.Object.enva_codigo[1]		=  400
end event
...
IF rb_bin.checked THEN
	l_s_titulo	= 'Calculo de Numerales por Bulto'
ELSE
	l_s_titulo	= 'Calculo de Numerales por Kilos'
END IF	

istr_info.titulo	= 'Movimiento de Fruta  Granel '+l_s_titulo 

OpenWithParm(vinf, istr_info)

IF rb_bin.checked THEN
	vinf.dw_1.DataObject = "dw_info_numerales_bultos"
ELSE
	vinf.dw_1.DataObject = "dw_info_numerales_kilos"
END IF	
...
IF isnull(ii_EnvaCodigo) THEN
	Messagebox("Atención","Debe seleccionar el Envase", StopSign!)
	Return -1
END IF

IF isnull(ii_envatipoen) THEN
	Messagebox("Atención","Debe seleccionar el Tipo Envase", StopSign!)
	Return -1
END IF

fila = vinf.dw_1.Retrieve(ii_cliente, ii_planta, ii_periodo, ii_Productor, ii_TipoProd, &
									  ii_especie, ii_variedad, ii_EnvaTipoen, ii_EnvaCodigo)
...
ELSE
	em_variedad.Text			=	lstr_busq.argum[3]
	sle_variedad.Text			=	lstr_busq.argum[4]
	ii_variedad					=	integer(lstr_busq.argum[3])
END IF
...
idwc_envases.Retrieve(Integer(data))
