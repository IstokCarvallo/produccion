...
type variables
Integer	Planta, faena, Estado
String		Rut, Paterno, Materno, Nombre, Direccion, Observacion, CodigoPersonal
Long  		Codigo, Tarja
Date		Ingreso, Fin
end variables
...
select pers_codigo,pers_rutemp, pers_apepat, pers_apemat, pers_nombre, 
		pers_direcc, pers_estado,pers_fecing, pers_fecfin, pers_nrotar, 	
		pers_observ
  into :CodigoPersonal, :Rut, :Paterno, :Materno, :Nombre,
  		:Direccion, :Estado, :Ingreso, :Fin, :Tarja, 
		:Observacion
...
IF at_Transaccion.SQLCode = -1 THEN
	F_ErrorBaseDatos(at_Transaccion, "Lectura de Entidades")
	lb_Retorno	=	False
