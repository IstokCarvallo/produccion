$PBExportHeader$w_maed_spro_orden_proceso_comercial.srw
$PBExportComments$Ventana de mantención de Orden de Proceso de Fruta Granel.
forward
global type w_maed_spro_orden_proceso_comercial from w_mant_encab_deta
end type
type cb_detalle from commandbutton within w_maed_spro_orden_proceso_comercial
end type
type cb_acepta from commandbutton within w_maed_spro_orden_proceso_comercial
end type
type st_1 from statictext within w_maed_spro_orden_proceso_comercial
end type
type dw_6 from uo_dw within w_maed_spro_orden_proceso_comercial
end type
type dw_3 from uo_dw within w_maed_spro_orden_proceso_comercial
end type
type dw_5 from uo_dw within w_maed_spro_orden_proceso_comercial
end type
type dw_4 from uo_dw within w_maed_spro_orden_proceso_comercial
end type
end forward
...
integer width = 3849
integer height = 2048
string title = "Orden de ReProceso"
string menuname = ""
...
global w_maed_spro_orden_proceso_comercial w_maed_spro_orden_proceso_comercial
...
w_mant_deta_ordenproceso_comercial				iw_mantencion_1
...
public function string uf_file_last_mod (string as_filename)
end prototypes
...
If ii_TipoOrd = 5 Then
	iuo_lotesfrutagranel.SaldoExistenciaComercial(Integer(istr_Mant.Argumento[2]), &
	li_Vari_codigo,ll_Prod_codigo, ls_frio_tipofr,li_pefr_codigo, SqlCa)
	
	If iuo_lotesfrutagranel.SaldoExistenciaComercial > 0 Then
		dw_1.Object.Saldo[il_Fila]	=	iuo_lotesfrutagranel.SaldoExistenciaComercial - ll_Bultos
	End If
Else
...
on w_maed_spro_orden_proceso_comercial.create
int iCurrent
call super::create
...
on w_maed_spro_orden_proceso_comercial.destroy
call super::destroy
		...
		DO
			IF dw_3.Retrieve(Integer(istr_Mant.Argumento[1]), &
				...
				respuesta = MessageBox("Error en Base de Datos", "No es posible conectar la Base de Datos.", &
												Information!, RetryCancel!)
			ELSE
				dw_3.SetFilter("orpr_tipord = " + String(ii_TipoOrd))
				dw_3.Filter()
				
				dw_1.SetFilter("orpr_tipord = " + String(ii_TipoOrd))
				dw_1.Filter()
...
event ue_antesguardar;call super::ue_antesguardar;Long			ll_Fila, ll_Numero, ll_NroProc, ll_secuencia
...
FOR ll_Fila = 1 TO dw_1.RowCount()
	IF dw_1.GetItemStatus(ll_Fila, 0, Primary!) = NewModified! Or dw_1.GetItemStatus(ll_Fila, 0, Primary!) = New! THEN
...
istr_info.titulo	= "INFORME REPROCESOS DIARIOS"
istr_info.copias	= 1

OpenWithParm(vinf,istr_info)
DataWindowChild	ldwc_especies
vinf.dw_1.DataObject = "dw_info_programa_procesos_diarios_comer"
...
type dw_1 from w_mant_encab_deta`dw_1 within w_maed_spro_orden_proceso_comercial
integer x = 32
integer y = 1052
integer width = 3223
integer height = 836
	...
	CASE "b_detalle"
		...
		ELSE
			...
			ELSE
				...
				IF ii_TipoOrd = 5 THEN
					IF iuo_lotesfrutagranel.SaldoExistenciaComercial > 0 OR ll_bultos > 0 THEN
						dw_1.Object.Saldo[il_Fila]	=	iuo_lotesfrutagranel.SaldoExistenciaComercial - ll_bultos
...
type dw_2 from w_mant_encab_deta`dw_2 within w_maed_spro_orden_proceso_comercial
integer x = 37
integer y = 40
integer width = 3081
integer height = 280
...
type pb_nuevo from w_mant_encab_deta`pb_nuevo within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 40
long backcolor = 553648127
end type

type pb_eliminar from w_mant_encab_deta`pb_eliminar within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 50
long backcolor = 553648127
end type

type pb_grabar from w_mant_encab_deta`pb_grabar within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 60
long backcolor = 553648127
end type

type pb_imprimir from w_mant_encab_deta`pb_imprimir within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 70
long backcolor = 553648127
end type

type pb_salir from w_mant_encab_deta`pb_salir within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 80
long backcolor = 553648127
end type

type pb_ins_det from w_mant_encab_deta`pb_ins_det within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 20
long backcolor = 553648127
end type

type pb_eli_det from w_mant_encab_deta`pb_eli_det within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 100
long backcolor = 553648127
end type

type pb_buscar from w_mant_encab_deta`pb_buscar within w_maed_spro_orden_proceso_comercial
integer x = 3342
integer taborder = 30
long backcolor = 553648127
end type
