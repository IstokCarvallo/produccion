$PBExportHeader$uo_seleccion_certificadora.sru
$PBExportComments$Objeto público para selección de Empresas Certificadoras, Todas o Consolidada
forward
global type uo_seleccion_certificadora from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_certificadora
end type
type cbx_todos from checkbox within uo_seleccion_certificadora
end type
type dw_seleccion from datawindow within uo_seleccion_certificadora
end type
end forward
...
global uo_seleccion_certificadora uo_seleccion_certificadora

type variables
DataWindowChild	idwc_Seleccion

uo_Certificadora	iuo_Certificadora
...
on uo_seleccion_certificadora.create
...
on uo_seleccion_certificadora.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_certificadoras'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'cert_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'cert_codigo'
...
IF	idwc_Seleccion.Retrieve() = 0 THEN
	...
	SetNull(Nombre)
ELSE
	idwc_Seleccion.SetSort("cert_nombre A")
	idwc_Seleccion.Sort()
	...
	Codigo				=	-1
	Nombre				=	'Todas'
	iuo_Certificadora	=	Create uo_Certificadora
...
IF iuo_Certificadora.Existe(Integer(data), True, sqlca) THEN
	Codigo	=	iuo_Certificadora.Codigo
	Nombre	=	iuo_Certificadora.Nombre
ELSE
