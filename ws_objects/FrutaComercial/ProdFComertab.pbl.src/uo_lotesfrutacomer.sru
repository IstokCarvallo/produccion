...
Date			ld_Fecham
String		TipoFrio, NomPlanta, NomEspecie, GrupoCalibre, Calibre, calidad
end variables

forward prototypes
public function boolean obtienesaldocamara (integer ai_planta, integer ai_camara, ref long al_saldo, boolean ab_mensaje, transaction at_transaccion)
...
public function boolean obtienesaldocamara (integer ai_planta, integer ai_camara, ref long al_saldo, boolean ab_mensaje, transaction at_transaccion);Boolean	lb_Retorno	=	True
...
IF lb_Retorno THEN
	IF ai_secuen <> -1 THEN
		SELECT	lofc_pltcod, lofc_espcod, lofc_lotefc, lfcd_secuen, lfcd_nturno,
					...
					lfcd_Kilnet, lfcd_kilpro, lfcd_tipool, cale_calida
			INTO	:Planta, :Especie, :Numero, :Secuencia, :Turno, :ld_Fecham, :Variedad,
					:Productor, :TipoFrio, :PeriodoFrio, :Tipoenvase, :Envase, :Calibre,
					:Grupocalibre, :TotalBulto, :TotalNeto, :KilosPromed, :Tipool, :calidad
			...
			USING at_Transaccion;
	ELSE
		SELECT	DISTINCT lofc_pltcod, lofc_espcod, lofc_lotefc, vari_codigo, prod_codigo, 
					frio_tipofr, pefr_codigo
			INTO	:Planta, :Especie, :Numero, :Variedad, :Productor, 
					:TipoFrio, :PeriodoFrio
			FROM	dbo.spro_lotesfrutacomdeta
			WHERE	lofc_pltcod	=	:ai_Planta
			AND	lofc_espcod	=	:ai_Especie
			AND	lofc_lotefc	=	:ai_Numero
			AND   :ai_secuen in (lfcd_secuen, -1)
			USING at_Transaccion;
...
//	FROM	dbo.spro_lotesfrutagrandeta lfd,dbo.spro_lotesfrutagranel lfg,
//			dbo.spro_camaraexistefg cam
