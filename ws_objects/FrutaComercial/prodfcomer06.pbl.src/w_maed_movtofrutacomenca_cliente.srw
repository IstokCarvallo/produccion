$PBExportHeader$w_maed_movtofrutacomenca_cliente.srw
$PBExportComments$Recepción - Despacho de Fruta Comercial.
forward
global type w_maed_movtofrutacomenca_cliente from w_mant_encab_deta_csd
end type
type dw_6 from datawindow within w_maed_movtofrutacomenca_cliente
end type
type dw_5 from datawindow within w_maed_movtofrutacomenca_cliente
end type
type cb_guisii from commandbutton within w_maed_movtofrutacomenca_cliente
end type
type cb_capturadatos from commandbutton within w_maed_movtofrutacomenca_cliente
end type
type dw_exideta from datawindow within w_maed_movtofrutacomenca_cliente
end type
type dw_exiencab from datawindow within w_maed_movtofrutacomenca_cliente
end type
type dw_exidetaborra from datawindow within w_maed_movtofrutacomenca_cliente
end type
type dw_exismovtodetanulos from datawindow within w_maed_movtofrutacomenca_cliente
end type
type tab_1 from tab within w_maed_movtofrutacomenca_cliente
end type
...
type productoresxlote from structure within w_maed_movtofrutacomenca_cliente
end type
type str_envase from structure within w_maed_movtofrutacomenca_cliente
end type
type str_productores_envases from structure within w_maed_movtofrutacomenca_cliente
end type
end forward
...
global w_maed_movtofrutacomenca_cliente w_maed_movtofrutacomenca_cliente
...
public subroutine wf_buscacliente ()
...
IF ib_Conectadoexistencia THEN
	...
	IF luo_existencia.Mesproceso > dw_2.Object.mfco_fecmov[1] THEN
		Message.DoubleParm = -1
		li_retorno = 2
		is_error = 'Problema con Fecha de Proceso Mayor a Fecha de Movimiento'
		Return 2
	END IF	
	...
	IF luo_existencia.count <> 0 THEN
		li_retorno = 2
		is_error = 'Problema con Encabezado de Movimiento de Existencia, Ya existe'
		Return 2
	END IF
	...
	FOR ll_general = 1 TO dw_3.RowCount()
		...
		ll_prod_codigo = Long(dw_2.Object.clpr_rut[1])
		
		ls_productor = String(ll_prod_codigo,'0000000000')
		
		IF Not luo_existencia.existecliente2(ls_productor,True,sqlexi) THEN
			Message.DoubleParm = -1
			li_retorno = 2
			is_error = 'Problema con los Productores en Existencia'
			RETURN 2
		ELSE	
		...
		IF luo_existencia.prod = '' OR isnull(luo_existencia.prod) THEN
			...
			Messagebox("Atención","Cliente NO Existe en Existencia")
			li_retorno = 2
			Return 2
		END IF	
		...
		IF ls_productorant <> ls_productor THEN
			...
			dw_exiencab.Object.mden_observ[ll_fila_nea] 	= 	'Venta a Terceros Comercial'
		...
		IF Not iuo_calicosechero.Existe(li_enva_tipoen,li_enva_codigo,ls_calidad,True,sqlca) THEN
			Message.DoubleParm = -1
			li_retorno = 2
			is_error = 'Problema con los Envases en Existencia, NO Existe'
			RETURN 2
		ELSE
...
event ue_despuesborrar();Long 		ll_numero, respuesta, ll_nueva, ll_fila, ll_docrel, li_tipdoc
...
ELSE
	...
	dw_2.Object.Prod_codigo_1[1] = Long(lstr_busq.argum[6])
...
IF as_Columna <> "clpr_rut" AND &
	(dw_2.Object.clpr_rut[1] = '' OR IsNull(dw_2.Object.clpr_rut[1])) THEN
	lb_Estado = False
END IF
...
public subroutine wf_buscacliente ();Str_busqueda	lstr_busq
...
If tab_1.SelectedTab = 1 Then
	...
	Else
		...
		istr_mant.Argumento[2]		=	'33'
		
		OpenWithParm(w_mant_deta_movtofrutcomer_tercero, istr_mant)
	End If
...
on w_maed_movtofrutacomenca_cliente.create
int iCurrent
call super::create
...
on w_maed_movtofrutacomenca_cliente.destroy
call super::destroy
destroy(this.dw_6)
destroy(this.dw_5)
		...
		IF gi_admenvase <> 1 THEN
			...
			IF il_conexiste = 1 THEN
				...
				IF Conexionexistencia() THEN
					IF ib_ModificacionEnvase THEN
						...
						IF li_retorno = 2 THEN
							...
							is_correo,'Problema Control de Envases Despacho Venta Terceros Fruta Comercial','Movimiento N° '+ls_numero+' Con problemas, Error '+is_error)
							is_error = ''	
						END IF		
				...
				ELSE
					...
					is_correo,'Problema Control de Envases Despacho Venta Terceros Fruta Comercial','Movimiento N° '+ls_numero+' Con problemas, Error en Conexión con Base de Existencia')
...
type dw_1 from w_mant_encab_deta_csd`dw_1 within w_maed_movtofrutacomenca_cliente
boolean visible = false
integer x = 142
integer y = 1012
integer width = 4608
...
type dw_2 from w_mant_encab_deta_csd`dw_2 within w_maed_movtofrutacomenca_cliente
integer x = 768
integer y = 32
integer width = 3095
integer height = 844
integer taborder = 10
string dataobject = "dw_mant_movtofrutacomenca_clientes"
end type
...
type pb_nuevo from w_mant_encab_deta_csd`pb_nuevo within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 256
integer taborder = 60
end type
...
type pb_eliminar from w_mant_encab_deta_csd`pb_eliminar within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 432
integer taborder = 70
end type

type pb_grabar from w_mant_encab_deta_csd`pb_grabar within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 616
integer taborder = 80
end type

type pb_imprimir from w_mant_encab_deta_csd`pb_imprimir within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 796
integer taborder = 90
end type

type pb_salir from w_mant_encab_deta_csd`pb_salir within w_maed_movtofrutacomenca_cliente
integer x = 4727
integer y = 972
integer taborder = 100
end type

type pb_ins_det from w_mant_encab_deta_csd`pb_ins_det within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 1488
integer taborder = 30
end type

type pb_eli_det from w_mant_encab_deta_csd`pb_eli_det within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 1664
integer taborder = 40
end type

type pb_buscar from w_mant_encab_deta_csd`pb_buscar within w_maed_movtofrutacomenca_cliente
integer x = 4736
integer y = 72
integer taborder = 50
end type
...
event doubleclicked;call super::doubleclicked;IF Row > 0 THEN
	il_fila = row
	This.SetRow(il_fila)
	w_maed_movtofrutacomenca_Cliente.TriggerEvent("ue_modifica_detalle")
END IF
RETURN 0
end event
...
CHOOSE CASE key
	CASE KeyEnter!
		w_maed_movtofrutacomenca_cliente.PostEvent("ue_modifica_detalle")
	...
	CASE KeyDownArrow!, KeyUpArrow!, KeyPageUp!, KeyPageDown!, KeyEnd!, KeyHome!
		w_maed_movtofrutacomenca_cliente.PostEvent("ue_seteafila")
...
event doubleclicked;call super::doubleclicked;w_maed_movtofrutacomenca_Cliente.TriggerEvent("ue_modifica_detalle")
