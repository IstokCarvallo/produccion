$PBExportHeader$w_busc_ordenretiroventa.srw
forward
global type w_busc_ordenretiroventa from w_busqueda
end type
end forward
...
integer width = 3506
integer height = 1768
string title = "Búsqueda de Ordenes de Retiro"
boolean resizable = false
end type
global w_busc_ordenretiroventa w_busc_ordenretiroventa
...
on w_busc_ordenretiroventa.create
call super::create
end on

on w_busc_ordenretiroventa.destroy
call super::destroy
end on
...
is_ordena		=	'Nº Retiro:oret_numero,Cliente:clpr_nombre,' + &
						'Fecha:oret_fecret'
					  
IF dw_1.Retrieve(integer(istr_busq.argum[1])) > 0 THEN
	IF integer(istr_busq.argum[2]) = 2 THEN
	   dw_1.SetFilter("odfc_tipret = 2")
		dw_1.Filter()
	END IF	
	dw_1.SetFocus()
...
istr_busq.argum[2]	= String(dw_1.GetItemNumber(dw_1.GetRow(),"oret_numero"))
...
type pb_insertar from w_busqueda`pb_insertar within w_busc_ordenretiroventa
...
type dw_1 from w_busqueda`dw_1 within w_busc_ordenretiroventa
integer x = 82
integer y = 724
integer width = 2866
integer height = 876
integer taborder = 60
string dataobject = "dw_mues_spro_ordenretiroenca"
end type

type pb_salir from w_busqueda`pb_salir within w_busc_ordenretiroventa
integer x = 3058
integer y = 1388
end type
...
type tab_1 from w_busqueda`tab_1 within w_busc_ordenretiroventa
integer x = 119
integer y = 60
integer width = 2510
