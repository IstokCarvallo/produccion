$PBExportHeader$w_mant_deta_movtofrutcomer_desp_cliente.srw
forward
global type w_mant_deta_movtofrutcomer_desp_cliente from w_mant_detalle_csd
end type
end forward
...
integer width = 2542
integer height = 1992
string title = "Detalle de Traspaso Ventas Cliente"
end type
global w_mant_deta_movtofrutcomer_desp_cliente w_mant_deta_movtofrutcomer_desp_cliente
...
public function boolean existeenorden (integer ai_tipord, long al_orden, integer ai_especie)
end prototypes
...
ELSEIF sqlca.SQLCode = 100 THEN
	...
	lb_Retorno	=	False
ELSEIF Not IsNull(ai_Cantidad) AND ai_Cantidad > 0 AND ai_Cantidad < li_Saldo THEN
...
public function boolean existeenorden (integer ai_tipord, long al_orden, integer ai_especie);Boolean lb_Retorno = TRUE
...
on w_mant_deta_movtofrutcomer_desp_cliente.create
call super::create
end on

on w_mant_deta_movtofrutcomer_desp_cliente.destroy
call super::destroy
end on
...
ELSEIF Not istr_mant.Agrega AND Not istr_mant.Borra THEN
	...
	iuo_Lote.Existe(iuo_PltaLote.Codigo, iuo_Especie.Codigo, &
						dw_1.object.lofc_lotefc[il_fila], &
...
event open;/* 
	Argumentos
		...
		istr_Mant.Argumento[9]	=	Tipo Documento
...
type pb_ultimo from w_mant_detalle_csd`pb_ultimo within w_mant_deta_movtofrutcomer_desp_cliente
end type

type pb_siguiente from w_mant_detalle_csd`pb_siguiente within w_mant_deta_movtofrutcomer_desp_cliente
end type

type pb_anterior from w_mant_detalle_csd`pb_anterior within w_mant_deta_movtofrutcomer_desp_cliente
end type

type pb_primero from w_mant_detalle_csd`pb_primero within w_mant_deta_movtofrutcomer_desp_cliente
end type

type pb_cancela from w_mant_detalle_csd`pb_cancela within w_mant_deta_movtofrutcomer_desp_cliente
integer x = 2245
integer y = 408
end type

type pb_acepta from w_mant_detalle_csd`pb_acepta within w_mant_deta_movtofrutcomer_desp_cliente
integer x = 2245
integer y = 232
end type

type pb_salir from w_mant_detalle_csd`pb_salir within w_mant_deta_movtofrutcomer_desp_cliente
integer x = 2245
integer y = 580
end type

type dw_1 from w_mant_detalle_csd`dw_1 within w_mant_deta_movtofrutcomer_desp_cliente
integer x = 0
integer y = 0
integer width = 2153
integer height = 1660
string dataobject = "dw_mant_movtofrutacomdeta_desp_cliente"
end type
...
CHOOSE CASE ls_Columna
	CASE "cama_codigo"
		...
		ELSE
			IF iuo_TipoMovto.Sentido = 2 THEN
				...
				This.Object.mfcd_bulent[il_Fila]) THEN
					This.SetItem(il_Fila, ls_Columna, Integer(ls_Nula))
	...
	CASE "lofc_espcod"
		...
		ELSEIF Duplicado(ls_Columna, Data) THEN
			...
			RETURN 1
		ELSE
			IF Not Existeenorden(integer(istr_mant.Argumento[9]), Long(istr_mant.Argumento[10]),Integer(data)) THEN
				This.SetItem(il_Fila, ls_Columna, Integer(ls_Nula))
	...
	CASE "lofc_lotefc"
		IF Not iuo_Lote.Existe(iuo_PltaLote.Codigo, &
										iuo_Especie.Codigo, Integer(Data), &
										dw_1.Object.lfcd_secuen[il_fila],True, sqlca) THEN
			This.SetItem(il_Fila, ls_Columna, Integer(ls_Nula))
		...
		ELSEIF Duplicado(ls_Columna, Data) THEN
			...
			RETURN 1
		ELSE
			IF Not Existeenorden(integer(istr_mant.Argumento[9]), Long(istr_mant.Argumento[10]),dw_1.Object.lofc_espcod[1]) THEN
				This.SetItem(il_Fila, ls_Columna, Integer(ls_Nula))
			  RETURN 1
			END IF
			IF iuo_TipoMovto.Sentido = 2 THEN
			...
			This.SetItem(il_Fila, "prod_codigo", iuo_Lote.Productor)
			...
			ELSE
			   This.SetItem(il_Fila, "enva_tipoen", iuo_lote.tipoenvase)	
			END IF	
			...
			ELSE	
				This.SetItem(il_Fila, "enva_codigo", iuo_lote.envase)
	...
	CASE "lfcd_secuen"
		IF Not iuo_Lote.Existe(iuo_PltaLote.Codigo, &
										iuo_Especie.Codigo, dw_1.Object.lofc_lotefc[il_fila], &
										Integer(data),True, sqlca) THEN
			This.SetItem(il_Fila, ls_Columna, Integer(ls_Nula))
	...
	CASE "mfcd_bulent"
		...
		This.Object.mfcd_kgnent[il_Fila]	=	Integer(Data) * iuo_Lote.KilosPromed
