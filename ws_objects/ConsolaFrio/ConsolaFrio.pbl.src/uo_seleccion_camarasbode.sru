$PBExportHeader$uo_seleccion_camarasbode.sru
$PBExportComments$Objeto público para selección de Planta, Todas o Consolidada
forward
global type uo_seleccion_camarasbode from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_camarasbode
end type
type cbx_todos from checkbox within uo_seleccion_camarasbode
end type
type dw_seleccion from datawindow within uo_seleccion_camarasbode
end type
end forward
...
global uo_seleccion_camarasbode uo_seleccion_camarasbode
...
Integer		Codigo, Planta, TipoFrio
String			Nombre 
end variables
...
IF idwc_Seleccion.Retrieve(ai_Planta) = 0 THEN
	MessageBox("Atención", "No existen Camaras para Planta seleccionada.")
...
on uo_seleccion_camarasbode.create
...
on uo_seleccion_camarasbode.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_camarasbode'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'cama_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'cama_codigo'
...
If	idwc_Seleccion.Retrieve(-1) = 0 Then
	...
	SetNull(Nombre)
Else
	idwc_Seleccion.SetSort("cama_nombre A")
	idwc_Seleccion.Sort()
...
IF iuo_CamarasBode.Existe(Planta, Integer(data), True, sqlca) THEN
	Codigo	=	iuo_CamarasBode.Camara
	Nombre	=	iuo_CamarasBode.NombreCamara
	TipoFrio	=	iuo_CamarasBode.TipoFrio
ELSE
