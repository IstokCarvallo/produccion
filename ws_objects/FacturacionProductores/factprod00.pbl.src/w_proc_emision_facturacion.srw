...
integer x = 247
integer width = 3168
string text = "Emision Factura Mensual Productores"
end type
...
For ll_Fila = 1 To dw_1.RowCount()
	If dw_1.IsSelected(ll_Fila) Then
		...
		Else
			...
			luo_Planta.Existe(ll_Planta, True, SQLCA)
			...
			If li_Resultado = 1 Then
				If dw_1.Object.faen_enviado[ll_Fila] = 0 Then
					If UpperBound(luo_Productor.Mail) > 0 And luo_Productor.Mail[1] <> '' Then 
						...
						If Not IsNull(luo_Factura.GGN) And luo_Factura.GGN <> '' Then  ls_Texto += String(luo_Factura.KilosGGN, '#,##0.00')  + ' Kilos fruta Certificada GLOBALG.A.P. GGN:' + luo_Factura.GGN + ' ~n'
