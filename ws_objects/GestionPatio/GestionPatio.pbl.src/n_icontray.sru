...
lstr_data.hIcon	= Send(Handle(aw_window), WM_GETICON, ICON_SMALL, 0)
lstr_data.szTip	= aw_window.title + Char(0)
...
// load icon
ll_handle = ExtractIconA(Handle(GetApplication()), as_filename, aui_iconindex)
...
CHOOSE CASE Lower(Right(as_imagename, 4))
	CASE ".ico"
		ll_handle = LoadImageA(0, as_imagename, IMAGE_ICON, 0, 0, LR_LOADFROMFILE + LR_DEFAULTSIZE)
	CASE ".cur", ".ani"
		ll_handle = LoadImageA(0, as_imagename, IMAGE_CURSOR, 0, 0, LR_LOADFROMFILE + LR_DEFAULTSIZE)
	CASE ELSE
END CHOOSE
...
lstr_data.szTip	= as_newtip + Char(0)
...
public function boolean of_balloon_tip (window aw_window, string as_title, string as_info, icon dwinfoflag);// modify window icon tip in the system tray
...
lstr_data.szInfoTitle	= as_title + Char(0)
lstr_data.szInfo			= as_info + Char(0)
lstr_data.uTimeout		= 15000
