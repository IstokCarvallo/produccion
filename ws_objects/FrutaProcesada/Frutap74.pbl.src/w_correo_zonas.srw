...
type gb_2 from groupbox within w_correo_zonas
end type
...
string disabledname = "\Desarrollo 17\Imagenes\Botones\Email.png"
alignment htextalign = left!
end type
...
IF NOT isnull(istr_parms.string_arg[5]) THEN
	mAttach.PathName		=	Trim(istr_parms.string_arg[5])
	mAttach.FileName 		=  Trim(istr_parms.string_arg[5])
	mAttach.Position = len(imMsg.notetext) - 1		
	imMsg.AttachmentFile[2] = mAttach

	mAttach.PathName		=	Trim(istr_parms.string_arg[6])
	mAttach.FileName 		=  Trim(istr_parms.string_arg[6])
	mAttach.Position = len(imMsg.notetext) - 1		
	imMsg.AttachmentFile[3] = mAttach
END IF
IF NOT isnull(istr_parms.string_arg[7]) THEN
	mAttach.PathName		=	Trim(istr_parms.string_arg[7])
	mAttach.FileName 		=  Trim(istr_parms.string_arg[7])
	mAttach.Position = len(imMsg.notetext) - 1		
	imMsg.AttachmentFile[4] = mAttach
END IF
