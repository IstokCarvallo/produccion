$PBExportHeader$uo_seleccion_actividadestacion.sru
$PBExportComments$Objeto público para selección de Especie, Todos o Consolidado
forward
global type uo_seleccion_actividadestacion from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_actividadestacion
end type
type cbx_todos from checkbox within uo_seleccion_actividadestacion
end type
type dw_seleccion from datawindow within uo_seleccion_actividadestacion
end type
end forward
...
global uo_seleccion_actividadestacion uo_seleccion_actividadestacion

type variables
DataWindowChild	idwc_Seleccion

uo_actividadestacion	iuo_actividadestacion
...
on uo_seleccion_actividadestacion.create
...
on uo_seleccion_actividadestacion.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_actividadestacion'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'acti_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'acti_codigo'
...
IF	idwc_Seleccion.Retrieve() = 0 THEN
	MessageBox("Atención", "No existen Actividades en tabla respectiva")
	
	SetNull(Codigo)
	SetNull(Nombre)
ELSE
	idwc_Seleccion.SetSort("acti_nombre A")
	idwc_Seleccion.Sort()
	...
	Codigo			=	-1
	Nombre			=	'Todas'
	iuo_actividadestacion	=	Create uo_actividadestacion
...
IF iuo_actividadestacion.Existe(Integer(data), True, sqlca) THEN
	Codigo	=	iuo_actividadestacion.Codigo
	Nombre	=	iuo_actividadestacion.Nombre
ELSE
