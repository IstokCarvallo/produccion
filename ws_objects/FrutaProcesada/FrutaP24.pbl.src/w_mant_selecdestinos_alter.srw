$PBExportHeader$w_mant_selecdestinos_alter.srw
forward
global type w_mant_selecdestinos_alter from w_mant_encab_deta
end type
type cb_2 from uo_cbstandard within w_mant_selecdestinos_alter
end type
type cb_3 from uo_cbstandard within w_mant_selecdestinos_alter
end type
type cb_4 from uo_cbstandard within w_mant_selecdestinos_alter
end type
type dw_enca from uo_dw within w_mant_selecdestinos_alter
end type
type dw_pallets from uo_dw within w_mant_selecdestinos_alter
end type
type cb_agrega from uo_cbstandard within w_mant_selecdestinos_alter
end type
type cb_elimina from uo_cbstandard within w_mant_selecdestinos_alter
end type
type dw_4 from uo_dw within w_mant_selecdestinos_alter
end type
type dw_3 from uo_dw within w_mant_selecdestinos_alter
end type
type dw_5 from datawindow within w_mant_selecdestinos_alter
end type
type dw_6 from uo_dw within w_mant_selecdestinos_alter
end type
type cb_1 from commandbutton within w_mant_selecdestinos_alter
end type
end forward
...
global w_mant_selecdestinos_alter w_mant_selecdestinos_alter
...
datastore ids_despadestienca, ids_despaselecdeta, ids_despaselectsele
...
public function boolean existe_destino (integer ai_destino)
public function string nombre_destino (integer ai_destino)
end prototypes
...
IF il_total = 0 THEN
	FOR ll_fila = 1 TO dw_pallets.RowCount() 
		...
		For ll_numero = 1 to dw_5.Rowcount()
				...
				IF dw_5.Update(True, False) = 1 THEN			//despadestisele
					Commit;
...
IF Borrando THEN
	IF dw_pallets.Update(True, False) = 1 THEN 					//despadestienca
		IF dw_1.Update(True, False) = 1 THEN						//despadestideta
			IF dw_enca.Update(True, False) = 1 THEN				//despadestisele
				Commit;
	...
	END IF
ELSE
	IF dw_enca.Update(True, False) = 1 THEN		 				//despadestienca
		IF dw_1.Update(True, False) = 1 THEN						//despadestideta
			IF dw_pallets.Update(True, False) = 1 THEN			//despadestisele
				Commit;
...
IF ii_tipo = 1 THEN
	SELECT max(isnull(dese_numero,0)) 
	INTO  :ll_numero
	FROM dba.despadestienca
...
IF sqlca.SQLCode = -1 THEN
	F_errorbasedatos(sqlca,"Lectura tabla despadestienca")
END IF
...
public function boolean existe_destino (integer ai_destino);Integer li_cont
...
ELSEIF li_cont = 0 THEN
	MessageBox("Atención", "Código Destino no ha sido creado.~r~r" + &
					"Ingrese o seleccione otro Código.")
	RETURN True
ELSE
...
public function string nombre_destino (integer ai_destino);String ls_nombre
...
on w_mant_selecdestinos_alter.create
int iCurrent
call super::create
...
on w_mant_selecdestinos_alter.destroy
call super::destroy
...
ELSE
	...
	dw_4.width	=	This.WorkSpaceWidth() - 1010
	maximo		=	dw_1.width
END IF

dw_2.x					= 37 + Round((maximo - dw_2.width) / 2, 0)
dw_enca.x				= 37 + Round((maximo - dw_enca.width) / 2, 0)
//dw_2.y					= 37
//
...
cb_1.x 					= dw_enca.x + 1300
cb_2.x					= li_posic_x - 80
cb_3.x					= li_posic_x - 80
cb_4.x					= li_posic_x - 80
end event

event ue_imprimir;//
Long		fila
integer li_planta, li_cliente, li_varirotula
String  ls_cliente, ls_planta, ls_numero, ls_hora, ls_fecha, ls_responsable, ls_embarque, ls_destino1, ls_destino2
...
IF ii_tipo = 1 THEN
	istr_info.titulo	= "PALLETS SELECCIONADOS POR DESTINO"
	istr_info.copias	= 1

	OpenWithParm(vinf,istr_info)
	
	vinf.dw_1.DataObject = "dw_info_despadesti2"
ELSE
	istr_info.titulo	= "PALLETS CON DESTINOS ALTERNATIVOS"
	istr_info.copias	= 1

	OpenWithParm(vinf,istr_info)
	
	vinf.dw_1.DataObject = "dw_info_despadesti2_alter"
END IF
...
ELSE
	...
	ls_destino1 	= nombre_destino(dw_enca.Object.dest_codig1[1])
	ls_destino2 	= nombre_destino(dw_enca.Object.dest_codig2[1])
	...
	vinf.dw_1.Modify("t_destino1.text = '" + ls_destino1 + "'")
	vinf.dw_1.Modify("t_destinoins.text = '" + ls_destino2 + "'")
	vinf.dw_1.Modify("t_recibidor.text = '" + dw_enca.Object.dest_recibi[1] + "'")
...
istr_busq.argum[10]	=  String(ii_tipo)
	...
	OpenWithParm(w_busc_despadestienca, istr_busq)
...
ii_tipo = 2//Integer(Message.StringParm)
...
dw_enca.SetTabOrder("dest_codig1",70)	
dw_enca.SetTabOrder("dest_recibi",0)
dw_enca.SetTabOrder("todos",80)
dw_enca.SetTabOrder("dest_codig2",100)	
dw_enca.SetTabOrder("dese_obser1",50)	
...
dw_enca.Modify("dese_respon.BackGroud.Color = " + String(rgb(255,255,255)))		
dw_enca.Modify("dest_recibi.BackGround.Color = " + String(RGB(166,180,210)))
dw_enca.Modify("dest_codig1.BackGround.Color = " + String(rgb(255,255,255)))
dw_enca.Modify("dese_respon.BackGround.Color = " + String(rgb(255,255,255)))
dw_enca.Modify("dest_codig2.BackGround.Color = " + String(rgb(255,255,255)))
dw_enca.Modify("dese_obser1.BackGround.Color = " + String(rgb(255,255,255)))

dw_enca.SetTabOrder("dest_recibi",0)
dw_enca.Modify("dest_recibi.BackGround.Color = " + String(RGB(166,180,210)))
dw_enca.Object.dest_recibi[1] = 'TODOS'
...
event ue_recuperadatos;call super::ue_recuperadatos;integer 	li_fila, li_cliente, li_planta, respuesta, li_ret, li_i, li_first, li_j, li_productor
...
DO
	...
	IF ii_tipo = 1 THEN
		li_fila = dw_2.retrieve(dw_enca.Object.Clie_codigo[1], ii_embqcod)
	ELSE
		li_fila = 1
	END IF
	...
	ELSEIF li_fila > 0 THEN
		...
		dw_enca.SetTabOrder("dese_numero",0)
		//dw_enca.SetTabOrder("embq_codigo",0)
		dw_enca.SetTabOrder("clie_codigo",0)
		dw_enca.SetTabOrder("plde_codigo",0)
		dw_enca.SetTabOrder("dese_fecsel",0)
		dw_enca.SetTabOrder("dese_respon",0)
		dw_enca.SetTabOrder("dest_recibi",0)
		dw_enca.SetTabOrder("dest_codig1",0)
		dw_enca.SetTabOrder("todos",0)
		dw_enca.SetTabOrder("dest_codig2",0)	
		dw_enca.SetTabOrder("dese_obser1",0)	
				
		dw_enca.Modify("dese_numero.BackGround.Color = " + String(RGB(166,180,210)))
		//dw_enca.Modify("embq_codigo.BackGround.Color = " + String(RGB(166,180,210)))	
		dw_enca.Modify("clie_codigo.BackGround.Color = " + String(RGB(166,180,210)))		
		dw_enca.Modify("plde_codigo.BackGround.Color = " + String(RGB(166,180,210)))		
		dw_enca.Modify("dese_fecsel.BackGround.Color = " + String(RGB(166,180,210)))	
		dw_enca.Modify("dese_respon.BackGround.Color = " + String(RGB(166,180,210)))	
		dw_enca.Modify("dest_recibi.BackGround.Color = " + String(RGB(166,180,210)))	
		dw_enca.Modify("dest_codig1.BackGround.Color = " + String(RGB(166,180,210)))	
		dw_enca.Modify("dest_codig2.BackGround.Color = " + String(RGB(166,180,210)))	
		dw_enca.Modify("dese_obser1.BackGround.Color = " + String(RGB(166,180,210)))	
		
		IF dw_enca.Object.dest_recibi[1] = 'TODOS' THEN
			dw_enca.Object.todos[1] = 1
		END IF	
			...
			//IF li_fila > 0 THEN
				FOR row = 1 to dw_1.RowCount()
					...
					li_ret = dw_4.retrieve(li_clie_codigo, li_plde_codigo, li_especie, li_variedad, ls_embalaje,&
								...
								li_inspeccion,li_destino,li_status,li_secuencia,li_rechazo)
...
type dw_1 from w_mant_encab_deta`dw_1 within w_mant_selecdestinos_alter
...
string dataobject = "dw_mant_despadestideta_alter"
...
type dw_2 from w_mant_encab_deta`dw_2 within w_mant_selecdestinos_alter
boolean visible = false
integer x = 50
integer y = 2132
integer width = 3205
...
type pb_nuevo from w_mant_encab_deta`pb_nuevo within w_mant_selecdestinos_alter
integer x = 3351
integer taborder = 120
end type

type pb_eliminar from w_mant_encab_deta`pb_eliminar within w_mant_selecdestinos_alter
integer x = 3351
...
ll_dese_numero = dw_enca.Object.dese_numero[1]
DECLARE limpia PROCEDURE FOR dba.sp_limpia_tabla_despadesti
...
type pb_grabar from w_mant_encab_deta`pb_grabar within w_mant_selecdestinos_alter
integer x = 3351
integer y = 640
...
type pb_imprimir from w_mant_encab_deta`pb_imprimir within w_mant_selecdestinos_alter
integer x = 3351
integer taborder = 150
end type

type pb_salir from w_mant_encab_deta`pb_salir within w_mant_selecdestinos_alter
integer x = 3351
integer taborder = 160
end type

type pb_ins_det from w_mant_encab_deta`pb_ins_det within w_mant_selecdestinos_alter
boolean visible = false
integer x = 3355
integer y = 1276
...
event pb_ins_det::clicked;call super::clicked;Integer li_fila, li_null

SetNull(li_null)

IF isnull(dw_enca.Object.dest_codig1[1]) OR dw_enca.Object.dest_codig1[1] = 0 THEN
	MessageBox("Atención", "Falta Ingresar Destino 1.~r~r" + &
					"seleccione un Destino.")
	dw_enca.SetItem(1, "dest_codig1", Integer(li_null))
	Return 1
END IF	

IF dw_enca.Object.todos[1] = 0 THEN
	IF isnull(dw_enca.Object.dest_recibi[1]) OR dw_enca.Object.dest_recibi[1] = '' THEN
		MessageBox("Atención", "Falta Ingresar Recibidor.~r~r" + &
						"seleccione un Recibidor.")
		dw_enca.SetItem(1, "dest_recibi", String(li_null))
		Return 1
	END IF	
END IF	

//IF isnull(dw_enca.Object.dest_codig2[1]) OR dw_enca.Object.dest_codig2[1] = 0 THEN
//	MessageBox("Atención", "Falta Ingresar Destino 2.~r~r" + &
//					"seleccione un Destino.")
//	dw_enca.SetItem(1, "dest_codig2", Integer(li_null))
//	Return 1					
//END IF
...
type pb_eli_det from w_mant_encab_deta`pb_eli_det within w_mant_selecdestinos_alter
boolean visible = false
integer x = 3355
integer y = 1444
...
type pb_buscar from w_mant_encab_deta`pb_buscar within w_mant_selecdestinos_alter
integer x = 3351
...
integer taborder = 11
boolean bringtotop = true
string dataobject = "dw_mant_despadesticenca_alter"
boolean maxbox = true
...
//				pb_eli_det.Enabled = False
	...
	CASE "dest_codig1"	
		IF Existe_destino(Integer(data)) THEN
			This.SetItem(row, "dest_codig1", Integer(ls_Nula))
			Return 1
		END IF
	...
	CASE "todos"	
		...
		IF data = '1' THEN
			...
			dw_enca.Object.dest_recibi[1] = 'TODOS'
		ELSEIF data = '0'THEN
			dw_enca.SetTabOrder("dest_recibi",80)
			dw_enca.Modify("dest_recibi.BackGround.Color = " + String(rgb(255,255,255)))
			dw_enca.Object.dest_recibi[1] = ''
		END IF	
			
			
//		IF Not Isnull(dw_enca.Object.dest_codig2[1]) THEN
//			IF dw_enca.Object.dest_codig2[1] = Integer(data) THEN
//				MessageBox("Atención", "Código Destino 1 no puede ser igual a Destino 2.~r~r" + &
//						"Ingrese o seleccione otro Destino.")
//				This.SetItem(row, "dest_codig1", Integer(ls_Nula))
//				Return 1		
//			END IF
//		END IF
...
//	CASE "dest_codig2"	
//		IF Existe_destino(Integer(data)) THEN
//			This.SetItem(row, "dest_codig2", Integer(ls_Nula))
//			Return 1
//		END IF
//		
//		IF Not Isnull(dw_enca.Object.dest_codig1[1]) THEN
//			IF dw_enca.Object.dest_codig1[1] = Integer(data) THEN
//				MessageBox("Atención", "Código Destino 2 no puede ser igual a Destino 1.~r~r" + &
//						"Ingrese o seleccione otro Destino.")
//				This.SetItem(row, "dest_codig2", Integer(ls_Nula))
//				Return 1		
//			END IF
...
string dataobject = "dw_mant_despadestisele_alter"
...
string dataobject = "dw_info_destinos_selec_pallets_alter"
boolean hscrollbar = true
boolean livescroll = true
end type
...
boolean titlebar = true
string title = "none"
string dataobject = "dw_mant_despadestisele2_alter"
boolean hscrollbar = true
...
event clicked;Integer 	li_plde_codigo, li_especie, li_variedad, li_etiqueta,li_clie_codigo, li_destino, li_inspeccion,&
			li_status,li_ret, li_i, li_first, li_j, li_productor, li_secuencia, li_rechazo
String ls_embalaje, ls_calibre
Date fecemb_desde, fecemb_hasta
Long	ll_cont,ll_dese_numero
...
IF Isnull(dw_enca.Object.dest_codig1[1]) OR dw_enca.Object.dest_codig1[1] = 0 THEN
	Messagebox("Atención","Falta el Ingreso del Destino Alternativo.", Exclamation!)
	Return
END IF	

IF Isnull(dw_enca.Object.dest_codig2[1]) OR dw_enca.Object.dest_codig2[1] = 0 THEN
	Messagebox("Atención","Falta Selección del Destino.", Exclamation!)
	Return
END IF	
...
ls_calibre		= 'TOD'
li_productor	= -1
li_destino		= dw_enca.Object.dest_codig2[1]
li_inspeccion  = -1
li_status  		= -1
li_secuencia 	= -1
li_rechazo		= 0
		
li_plde_codigo = dw_enca.object.plde_codigo[1]
		
ll_cont = dw_6.Retrieve(li_clie_codigo, li_plde_codigo, dw_enca.object.dese_numero[1],ii_tipo)

IF ll_cont > 0 THEN
	dw_pallets.Retrieve(li_clie_codigo, li_plde_codigo, dw_enca.object.dese_numero[1],ii_tipo)
END IF
...
fecemb_desde = Date(19000101)//dw_enca.Object.dese_fecsel[1]
fecemb_hasta = Today()
...
dw_1.Object.dese_secuen[1] = 1

IF li_ret > 0 THEN
	
	FOR li_i = 1 TO dw_4.RowCount()
		dw_4.object.dese_secuen[li_i] = dw_1.object.dese_secuen[1]
	NEXT
	
	IF isnull(dw_1.object.dece_tipsel[1]) THEN dw_1.object.dece_tipsel[1] = 2
		
	IF isnull(dw_1.object.espe_codigo[1]) THEN dw_1.object.espe_codigo[1] = li_especie
	IF isnull(dw_1.object.vari_codigo[1]) THEN dw_1.object.vari_codigo[1] = li_variedad
	IF isnull(dw_1.object.emba_codigo[1]) THEN dw_1.object.emba_codigo[1] = ls_embalaje
	IF isnull(dw_1.object.etiq_codigo[1]) THEN dw_1.object.etiq_codigo[1] = li_etiqueta
	IF isnull(dw_1.object.paen_calibr[1]) THEN dw_1.object.paen_calibr[1] = ls_calibre
	IF isnull(dw_1.object.prod_codigo[1]) THEN dw_1.object.prod_codigo[1] = li_productor
	IF isnull(dw_1.object.dest_codigo[1]) THEN dw_1.object.dest_codigo[1] = li_destino
	IF isnull(dw_1.object.stat_codigo[1]) THEN dw_1.object.stat_codigo[1] = li_status
	IF isnull(dw_1.object.dese_secdet[1]) THEN dw_1.object.dese_secdet[1] = li_secuencia
	IF isnull(dw_1.object.dese_rechaz[1]) THEN dw_1.object.dese_rechaz[1] = li_rechazo

	dw_1.object.clie_codigo[1] = li_clie_codigo
	dw_1.object.plde_codigo[1] = li_plde_codigo
	dw_1.object.dese_numero[1] = Long(istr_busq.argum[5])
	dw_1.object.dese_fecini[1] = fecemb_desde
	dw_1.object.dese_fecter[1] = Today()
ELSE
	Messagebox("Atención","No existe Pallets que cumplan con los parametros de busqueda", Exclamation!)
END IF
