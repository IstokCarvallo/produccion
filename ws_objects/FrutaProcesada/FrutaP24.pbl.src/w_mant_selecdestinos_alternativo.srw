$PBExportHeader$w_mant_selecdestinos_alternativo.srw
forward
global type w_mant_selecdestinos_alternativo from w_mant_encab_deta
end type
type cb_2 from uo_cbstandard within w_mant_selecdestinos_alternativo
end type
type cb_3 from uo_cbstandard within w_mant_selecdestinos_alternativo
end type
type cb_4 from uo_cbstandard within w_mant_selecdestinos_alternativo
end type
type dw_enca from uo_dw within w_mant_selecdestinos_alternativo
end type
type dw_pallets from uo_dw within w_mant_selecdestinos_alternativo
end type
type cb_agrega from uo_cbstandard within w_mant_selecdestinos_alternativo
end type
type cb_elimina from uo_cbstandard within w_mant_selecdestinos_alternativo
end type
type dw_4 from uo_dw within w_mant_selecdestinos_alternativo
end type
type dw_3 from uo_dw within w_mant_selecdestinos_alternativo
end type
type dw_5 from datawindow within w_mant_selecdestinos_alternativo
end type
type dw_6 from uo_dw within w_mant_selecdestinos_alternativo
end type
end forward
...
global w_mant_selecdestinos_alternativo w_mant_selecdestinos_alternativo
...
public subroutine borra_anterior ()
public function boolean existerecibidor (integer ai_recibidor)
end prototypes
...
public subroutine borra_anterior ();Long	ll_fila, ll_pallet
Integer	li_planta, li_cliente

FOR ll_fila = 1 TO dw_pallets.RowCount() 
	ll_pallet 	= dw_pallets.Object.paen_numero[ll_fila]
	li_planta 	= dw_pallets.Object.plde_codigo[ll_fila]
	li_cliente 	= dw_pallets.Object.clie_codigo[ll_fila]
	
	DELETE FROM dba.despadestisele
...
public function boolean existerecibidor (integer ai_recibidor);Integer li_cont
String	ls_nombre

SELECT count()
INTO :li_cont
FROM dba.consignatario
WHERE cons_codigo = :ai_recibidor;
...
IF sqlca.SQLCode = -1 THEN
	...
	RETURN True
ELSEIF li_cont = 0 THEN
	MessageBox("Atención", "Código consignatario no ha sido creado.~r~r" + &
					"Ingrese o seleccione otro Código.")
	RETURN True
ELSE
	SELECT cons_nombre
	...
	dw_enca.Object.dest_recibi[1] = ls_nombre
	
  Return False
END IF
end function

on w_mant_selecdestinos_alternativo.create
int iCurrent
call super::create
...
on w_mant_selecdestinos_alternativo.destroy
call super::destroy
...
ELSE
	...
	IF dw_enca.Object.paen_tipopa[1] = -1 THEN
		vinf.dw_1.Modify("t_tipopallet.text = '" + 'Ambas' + "'")
	ELSEIF dw_enca.Object.paen_tipopa[1] = 1 THEN
		vinf.dw_1.Modify("t_tipopallet.text = '" + 'Pallet' + "'")
	ELSE	
		vinf.dw_1.Modify("t_tipopallet.text = '" + 'Pucho' + "'")
	END IF	
...
dw_enca.Object.paen_tipopa[1] = -1
...
dw_enca.SetTabOrder("recibidor",0)
...
dw_enca.Modify("recibidor.BackGround.Color = " + String(RGB(166,180,210)))
...
integer 	li_plde_codigo, li_especie, li_variedad, li_etiqueta,li_clie_codigo, row, li_inspeccion,li_tipopallet,&
		...
		ELSE
			//IF li_fila > 0 THEN
				FOR row = 1 to dw_1.RowCount()
					...
					li_tipopallet = dw_enca.Object.paen_tipopa[1]
					...
					li_ret = dw_4.retrieve(li_clie_codigo, li_plde_codigo, li_especie, li_variedad, ls_embalaje,&
								...
								li_inspeccion,li_destino,li_status,li_secuencia,li_rechazo,li_tipopallet)
...
type dw_1 from w_mant_encab_deta`dw_1 within w_mant_selecdestinos_alternativo
string tag = ","
integer x = 32
integer y = 564
integer width = 3205
...
string dataobject = "dw_mant_despadestideta_alternativo"
...
event dw_1::buttonclicked;call super::buttonclicked;Integer 	li_plde_codigo, li_especie, li_variedad, li_etiqueta,li_clie_codigo, li_destino, li_inspeccion,li_tipopallet, &
...
CHOOSE CASE dwo.name
	...
	CASE "b_pallets" 
		...
		IF NOT isnull(THIS.object.dest_codigo[row]) THEN li_destino 	= dw_enca.Object.dest_codig2[1]
		...
		li_ret = dw_4.retrieve(li_clie_codigo, li_plde_codigo, li_especie, li_variedad, ls_embalaje, li_etiqueta,&
						...
						li_destino,li_status,li_secuencia,li_rechazo,li_tipopallet)
...
event dw_1::clicked;call super::clicked;Integer 	li_plde_codigo, li_especie, li_variedad, li_etiqueta,li_clie_codigo, li_destino, li_inspeccion,&
			li_status,li_ret, li_i, li_first, li_j, li_productor, li_secuencia,li_rechazo,li_tipopallet
String ls_embalaje, ls_calibre
Date fecemb_desde, fecemb_hasta
Long	ll_cont,ll_dese_numero
datawindowchild dwc_variedad
...
type dw_2 from w_mant_encab_deta`dw_2 within w_mant_selecdestinos_alternativo
...
type pb_nuevo from w_mant_encab_deta`pb_nuevo within w_mant_selecdestinos_alternativo
integer x = 3351
integer taborder = 120
end type

type pb_eliminar from w_mant_encab_deta`pb_eliminar within w_mant_selecdestinos_alternativo
integer x = 3351
...
type pb_grabar from w_mant_encab_deta`pb_grabar within w_mant_selecdestinos_alternativo
integer x = 3351
integer y = 640
...
type pb_imprimir from w_mant_encab_deta`pb_imprimir within w_mant_selecdestinos_alternativo
integer x = 3351
integer taborder = 150
end type

type pb_salir from w_mant_encab_deta`pb_salir within w_mant_selecdestinos_alternativo
integer x = 3351
integer taborder = 160
end type

type pb_ins_det from w_mant_encab_deta`pb_ins_det within w_mant_selecdestinos_alternativo
integer x = 3355
integer y = 1276
...
dw_1.object.dest_codigo[li_fila] = dw_enca.Object.dest_codig2[1]
...
type pb_eli_det from w_mant_encab_deta`pb_eli_det within w_mant_selecdestinos_alternativo
integer x = 3355
integer y = 1444
...
type pb_buscar from w_mant_encab_deta`pb_buscar within w_mant_selecdestinos_alternativo
integer x = 3351
...
//				pb_eli_det.Enabled = False
	...
	CASE "todos"	
		...
		IF data = '1' THEN
			...
			dw_enca.Modify("recibidor.BackGround.Color = " + String(RGB(166,180,210)))
			dw_enca.Object.dest_recibi[1] = 'TODOS'
			This.SetItem(row, "recibidor", Long(ls_Nula))
		ELSEIF data = '0'THEN
			dw_enca.SetTabOrder("recibidor",80)
			dw_enca.Modify("recibidor.BackGround.Color = " + String(rgb(255,255,255)))
			dw_enca.Object.dest_recibi[1] = ''
			This.SetItem(row, "recibidor", Long(ls_Nula))
		END IF
		
	CASE "recibidor"
		IF existerecibidor(Long(data)) THEN
			This.SetItem(row, "recibidor", Long(ls_Nula))
			Return 1
		END IF	
...
string dataobject = "dw_mant_despadestisele"
...
string dataobject = "dw_info_destinos_selec_pallets"
boolean hscrollbar = true
boolean livescroll = true
end type
...
boolean titlebar = true
string title = "none"
string dataobject = "dw_mant_despadestisele2"
boolean hscrollbar = true
