$PBExportHeader$w_maed_palletencab_recepcion_caja.srw
forward
global type w_maed_palletencab_recepcion_caja from w_maed_palletencab_trans
end type
type dw_3 from datawindow within w_maed_palletencab_recepcion_caja
end type
end forward
...
global w_maed_palletencab_recepcion_caja w_maed_palletencab_recepcion_caja
...
Integer		ii_CantPallets, ii_CantPuchos,ii_tipopal;
...
ElseIf Not IsNull(li_Existe) AND li_Existe > 0 Then
	MessageBox("Error","Pallet Ya Existe", Information!, Ok!)				
	Return False
End If

SELECT paen_estado,paen_ccajas,paen_pedesp,paen_tipopa
INTO  :li_Estado,:ll_cajas,:li_pedesp,:ii_tipopal
...
ElseIf Not IsNull(li_Estado) AND li_Estado = 2 Then
	MessageBox("Error","Pallet Ya Fue Recepcionado", Information!, Ok!)					
	Return False
ElseIf Isnull(li_pedesp) OR li_pedesp = 0 Then
	   ...
	   SELECT max(tpem_codigo)
		...
		If Long(ls_tpemcodigo) <> ll_cajas Then
				MessageBox("Error","Pallet No es Pedido Especial y No Contiene Total de Cajas", Information!, Ok!)					
				Return False			
		End If
End If
...
If ll_cajas > 0 Then
	...
	il_pallet = al_Pallet
Else
	MessageBox("Error","Pallet No Existe en Packing", Information!, Ok!)	
	Return False				
End If	
...
This.title = 'RECEPCION DESDE PACKING'
...
on w_maed_palletencab_recepcion_caja.create
int iCurrent
call super::create
...
on w_maed_palletencab_recepcion_caja.destroy
call super::destroy
destroy(this.dw_3)
end on
...
type dw_1 from w_maed_palletencab_trans`dw_1 within w_maed_palletencab_recepcion_caja
integer x = 78
integer y = 340
integer height = 1128
...
type dw_2 from w_maed_palletencab_trans`dw_2 within w_maed_palletencab_recepcion_caja
integer x = 471
integer width = 1467
integer height = 180
...
Choose Case ls_Columna
	Case "paen_numero"
		...
		If (Duplicado(Integer(istr_mant.argumento[3]),ll_NroPallet) Or &
			Not ExisteCapturaPallet(Integer(istr_mant.argumento[3]), Integer(istr_mant.argumento[1]), ll_NroPallet) ) Then
...
type pb_nuevo from w_maed_palletencab_trans`pb_nuevo within w_maed_palletencab_recepcion_caja
end type

type pb_eliminar from w_maed_palletencab_trans`pb_eliminar within w_maed_palletencab_recepcion_caja
boolean visible = false
end type

type pb_grabar from w_maed_palletencab_trans`pb_grabar within w_maed_palletencab_recepcion_caja
integer x = 3296
end type

event pb_grabar::clicked;Integer 	li_Fila, li_filanew, li_cliente, li_planta, li_tipoen
...
If Not IsNull(dw_2.Object.paen_numero[1]) Then 		
	If dw_2.rowcount() > 0 Then
		...
		If Integer(istr_mant.Argumento[20]) = 3 Then
			li_tipoen		= 1
		Else
			li_tipoen		= Integer(istr_mant.Argumento[20])
		End If
		
		DECLARE Traspaso_recepcion_caja PROCEDURE FOR dbo.Fproc_Recepcion_Pda_Caja			
			@Cliente			=	:li_Cliente,			      
			@Planta			=	:li_Planta,
			@pallet			=	:il_pallet,
			@numero  		=  :ll_numero,
			...
			@tipoen			=  :li_tipoen,
			@usuario		=	:gstr_us.Nombre,
			@computador	= 	:gstr_us.Computador,
			@tipopallet 		=  :ii_tipopal
		Using SQLCA;
		...
		Else
			Commit;
			Close 	Traspaso_recepcion_caja;
		End If
		
		w_maed_recfruprocee_particular_caja.TriggerEvent("ue_traedatos")
...
type pb_imprimir from w_maed_palletencab_trans`pb_imprimir within w_maed_palletencab_recepcion_caja
boolean visible = false
end type

type pb_salir from w_maed_palletencab_trans`pb_salir within w_maed_palletencab_recepcion_caja
end type
...
type pb_ins_det from w_maed_palletencab_trans`pb_ins_det within w_maed_palletencab_recepcion_caja
boolean visible = false
integer y = 1488
end type

type pb_eli_det from w_maed_palletencab_trans`pb_eli_det within w_maed_palletencab_recepcion_caja
boolean visible = false
end type

type pb_buscar from w_maed_palletencab_trans`pb_buscar within w_maed_palletencab_recepcion_caja
boolean visible = false
end type

type dw_histoencab from w_maed_palletencab_trans`dw_histoencab within w_maed_palletencab_recepcion_caja
integer x = 1637
integer y = 2144
end type

type dw_histofruta from w_maed_palletencab_trans`dw_histofruta within w_maed_palletencab_recepcion_caja
integer x = 0
integer y = 2020
end type
