$PBExportHeader$w_maed_palletencab_recepcion_rotulatrans.srw
forward
global type w_maed_palletencab_recepcion_rotulatrans from w_maed_palletencab_trans
end type
type dw_3 from datawindow within w_maed_palletencab_recepcion_rotulatrans
end type
type dw_4 from datawindow within w_maed_palletencab_recepcion_rotulatrans
end type
type dw_6 from datawindow within w_maed_palletencab_recepcion_rotulatrans
end type
type dw_5 from datawindow within w_maed_palletencab_recepcion_rotulatrans
end type
end forward
...
global w_maed_palletencab_recepcion_rotulatrans w_maed_palletencab_recepcion_rotulatrans
...
public subroutine buscaembalaje ();Integer	li_null
...
If lstr_busq.Argum[2] <> "" Then
	...
	dw_2.SetItem(1, "copa_codigo", Integer(lstr_busq.Argum[6]))
	dw_2.SetItem(1, "paen_ccajas", Integer(li_null))
	dw_2.SetItem(1, "tpem_codigo", String(li_null))
...
dw_emba.SetTransObject(sqlca)
If dw_emba.Retrieve(li_cliente, 'U006') = 0 Then dw_emba.Insertrow(0)
...
on w_maed_palletencab_recepcion_rotulatrans.create
int iCurrent
call super::create
...
on w_maed_palletencab_recepcion_rotulatrans.destroy
call super::destroy
destroy(this.dw_3)
destroy(this.dw_4)
destroy(this.dw_6)
...
event ue_antesguardar;Long		ll_fila, ll_inspec, ll_prod,ll_cajas,ll_fil1, ll_productor, ll_Numero, ll_fila2, ll_null
...
Date     ld_fecemb
Long		 ll_fila_P, ll_CajasDet
Integer	li_Secuencia,  li_paen_mixto, li_respue
...
dw_2.Object.stat_codig2[1]	=	1
dw_2.Object.stat_codig3[1]	=	1
dw_2.Object.stat_codig4[1]	=	1
dw_2.Object.stat_codig5[1]	=	1
dw_2.Object.stat_codig6[1]	=	1
...
If dw_2.Object.paen_ccajas[1] > ll_CajasDet Then
	li_respue	=	MessageBox("ADVERTENCIA","La Suma de Cajas del Detalle del Pallet~r" + &
							"es Menor a la Especificada.~r~rDesea Grabar.", Question!, YesNo!)
	If li_respue <> 1 Then
...
If dw_2.Object.paen_tipopa[1] = 1 Then
	If dw_2.Object.paen_ccajas[1] <> ll_CajasDet Then
		li_respue	=	MessageBox("ADVERTENCIA","La Suma de Cajas del Detalle del Pallet~r" + &
								"es Distinta a la Especificada.~r~rDesea Grabar.", Question!, YesNo!)
		If li_respue <> 1 Then
			Message.DoubleParm = -1
		End If	
	End If
...
Else
	...
	If gb_Repalletizado Then
		...
		If dw_2.Object.paen_inspec[1] = 1 Then
			...
			If Not duplicadoinspecdet(ll_inspec) Then
			...
			If Not duplicadoinspeccion(ll_inspec) Then
...
//	   istr_mant.argumento[7]	=	dw_1.Object.emba_codigo[il_fila] 
		...
		IF istr_mant2.Argumento[40] = '1' OR istr_mant.argumento[50] = '5' THEN	
			OpenWithParm(w_mant_deta_palletfruta_interfecemb_rotulatrans, istr_mant)
		ELSE	
			OpenWithParm(w_mant_deta_palletfruta_rotulatrans, istr_mant)
		END IF	
	END IF
END IF
end event
...
dw_2.Object.paen_numero.Color = RGB(0,0,0)
...
cliente_rotulado(Integer(istr_mant.argumento[1]))
...
dw_2.Object.esta_codigo[1]		=	2
dw_2.Object.paen_fecpro[1]	=	DateTime(istr_mant2.Argumento[51])
...
If dw_3.RowCount() > 0 Then
	...
	If ii_CantPallets > 0 AND dw_3.GetItemNumber(1, "pallets") = ii_CantPallets Then
		If Not lb_MensPallet Then
			MessageBox("Atención", "Se ha Completado la Cantidad de Pallets~rindicados en la Recepción")
			lb_MensPallet	=	True
		End If
	...
	If ii_CantPuchos > 0 AND dw_3.GetItemNumber(1, "puchos") = ii_CantPuchos Then
		If Not lb_MensPucho Then
			MessageBox("Atención", "Se ha Completado la Cantidad de Puchos~rindicados en la Recepción")
			lb_MensPucho	=	True
		End If
...
Else
	...
	istr_mant.argumento[58] = String(ExisteVariedad_Relacionada(Integer(istr_mant.Argumento[3]),integer(istr_mant.Argumento[4])))
...
DO
	...
	If ll_fila_e = -1 Then
		respuesta = MessageBox(	"Error en Base de Datos", "No es posible conectar la Base de Datos 1.", 	Information!, RetryCancel!)
	Else
...
type dw_1 from w_maed_palletencab_trans`dw_1 within w_maed_palletencab_recepcion_rotulatrans
string dataobject = "dw_mues_palletfruta_rotulatrans"
boolean righttoleft = false
end type

type dw_2 from w_maed_palletencab_trans`dw_2 within w_maed_palletencab_recepcion_rotulatrans
integer y = 60
integer width = 2930
integer height = 1020
end type
...
Choose Case ls_columna
	...
	Case "paen_numero"
		...
		If istr_mant.argumento[50] <> '3'  Then
		...
		Else
			...
			If existe_pallet_devolucion(data) Then
				dw_2.SetItem(1, "paen_numero", ll_null)
				Return 1
			End If
...
Choose Case ls_columna
	...
	Case "paen_tipopa"
		If data = '2' Then
			dw_2.SetItem(1, "tpem_codigo", String(ll_null))
		Else
			This.Object.paen_conpar[Row] = 0
			This.Object.paen_conpal[Row] = 0
		End If	
		
	Case "emba_codigo"
		If Not iuo_Embalajes.Existe(Integer(istr_mant.argumento[1]), Data, True, Sqlca) Then
//		If ExisteEmbalaje(data) = False Then
			This.SetItem(Row, "emba_nombre", "")
			This.SetItem(Row, "emba_codigo", String(ll_null))
			Return 1
		Else
			dw_emba.Retrieve(Integer(istr_mant.argumento[1]), iuo_Embalajes.Codigo)
			istr_mant.argumento[7]	=	iuo_Embalajes.Codigo
			istr_mant.argumento[8]	=	iuo_Embalajes.Nombre
		 	This.SetItem(Row, "tpem_codigo", String(ll_null))
			This.SetItem(Row, "emba_nombre", iuo_Embalajes.Nombre)
			This.Object.copa_codigo[Row] = iuo_Embalajes.BasePallet
		End If
			
	Case "tpem_codigo"
		If This.Object.paen_tipopa[Row] = 1 Then
			If ExisteTipoEmbalaje(data) = False Then
				This.SetItem(1, "emba_codigo", string(ll_null))
				Return 1
			Else
				dw_1.AcceptText()
				dw_1.GroupCalc()
				...
				If dw_1.RowCount() > 0 Then 
					If dw_1.Object.cajas[1] <> Long(istr_Mant.Argumento[11]) Then
						MessageBox("Error...","Cantidad de cajas no corresponde con el detalle.", StopSign!, OK!)
						Return 1
					End If
				End If				
			End If
...
event dw_2::clicked;call super::clicked;//tpem_codigo
end event

type pb_nuevo from w_maed_palletencab_trans`pb_nuevo within w_maed_palletencab_recepcion_rotulatrans
end type

type pb_eliminar from w_maed_palletencab_trans`pb_eliminar within w_maed_palletencab_recepcion_rotulatrans
end type

type pb_grabar from w_maed_palletencab_trans`pb_grabar within w_maed_palletencab_recepcion_rotulatrans
integer y = 684
end type

event pb_grabar::clicked;Parent.TriggerEvent("ue_guardar")
//w_maed_recfruprocee_particular_trans.TriggerEvent("ue_guardar")
w_maed_recfruprocee_particular_rotulatrans.pb_grabar.PostEvent(Clicked!)
end event

type pb_imprimir from w_maed_palletencab_trans`pb_imprimir within w_maed_palletencab_recepcion_rotulatrans
end type

type pb_salir from w_maed_palletencab_trans`pb_salir within w_maed_palletencab_recepcion_rotulatrans
end type
...
type pb_ins_det from w_maed_palletencab_trans`pb_ins_det within w_maed_palletencab_recepcion_rotulatrans
integer y = 1488
end type

type pb_eli_det from w_maed_palletencab_trans`pb_eli_det within w_maed_palletencab_recepcion_rotulatrans
end type

type pb_buscar from w_maed_palletencab_trans`pb_buscar within w_maed_palletencab_recepcion_rotulatrans
boolean visible = false
end type

type dw_histoencab from w_maed_palletencab_trans`dw_histoencab within w_maed_palletencab_recepcion_rotulatrans
end type

type dw_histofruta from w_maed_palletencab_trans`dw_histofruta within w_maed_palletencab_recepcion_rotulatrans
integer y = 1444
end type
