$PBExportHeader$w_gene_archivo_interplanta.srw
forward
global type w_gene_archivo_interplanta from window
end type
type uo_selplantas from uo_seleccion_plantas within w_gene_archivo_interplanta
end type
type uo_selcliente from uo_seleccion_clientesprod within w_gene_archivo_interplanta
end type
type ddlb_1 from dropdownlistbox within w_gene_archivo_interplanta
end type
type st_7 from statictext within w_gene_archivo_interplanta
end type
type cbx_var from checkbox within w_gene_archivo_interplanta
end type
type em_fecha from editmask within w_gene_archivo_interplanta
end type
type st_4 from statictext within w_gene_archivo_interplanta
end type
type st_3 from statictext within w_gene_archivo_interplanta
end type
type sle_mensa from singlelineedit within w_gene_archivo_interplanta
end type
type em_planilla from editmask within w_gene_archivo_interplanta
end type
type st_5 from statictext within w_gene_archivo_interplanta
end type
type dw_22 from datawindow within w_gene_archivo_interplanta
end type
type st_2 from statictext within w_gene_archivo_interplanta
end type
type st_1 from statictext within w_gene_archivo_interplanta
end type
type pb_salir from picturebutton within w_gene_archivo_interplanta
end type
type pb_grabar from picturebutton within w_gene_archivo_interplanta
end type
type st_6 from statictext within w_gene_archivo_interplanta
end type
type dw_1 from datawindow within w_gene_archivo_interplanta
end type
type dw_2 from datawindow within w_gene_archivo_interplanta
end type
end forward
...
global w_gene_archivo_interplanta w_gene_archivo_interplanta
...
ELSE
   ...
   dw_2.Sort()
	...
	ls_Registro 	 =	String(li_PldSag,'000')
	...
	FOR ll_Fila = 1 TO ll_Filas
		ls_Registro =	f_ClienteRotulado(uo_SelCliente.Codigo) //String(Integer(istr_mant.argumento[1]),'000')
		...
		ls_Registro	+=	String(dw_2.Object.destinoins[ll_Fila], '000')
		ls_Registro	+=	String(dw_2.Object.fechains[ll_Fila], 'YYYYMMDD')		
	...
	IF li_region <> 4 THEN
		ls_Archivo	= String(li_PldSag,'000') + String(ll_Numero,'00000') + ".INT"
	ELSE	
		ls_Archivo	= String(li_PldSag,'0000') + String(ll_Numero,'00000') + ".INT"
	END IF
...
IF (al_planilla <> 0) OR uo_SelPlantas.Codigo = 0 THEN
	...
	ELSEIF li_tiposa <> 11 THEN
		MessageBox("Atención", "Planilla S.A.G. Distinto a Interplanta.~r~rIngrese otro Número.", &
						Exclamation!, Ok!)
		pb_grabar.Enabled	= False
		em_planilla.SetFocus()
		RETURN False				
...
on w_gene_archivo_interplanta.create
...
on w_gene_archivo_interplanta.destroy
...
boolean enabled = false
string text = "Genera Archivo Plano Anexo Interplanta"
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
