$PBExportHeader$w_gene_deta_archivo_devolucion.srw
forward
global type w_gene_deta_archivo_devolucion from w_mant_detalle
end type
type cb_1 from commandbutton within w_gene_deta_archivo_devolucion
end type
end forward
...
global w_gene_deta_archivo_devolucion w_gene_deta_archivo_devolucion
...
SELECT 	pal.paen_tipopa, var.vari_nombre, pal.emba_codigo, pal.cate_codigo, 
   ...
   FROM dba.inspecpaldet as ins,dba.palletencab as pal,dba.variedades  as var,
	     dba.inspecpalenc as ine
   WHERE pal.clie_codigo = ins.clie_codigo  AND  
         ...
         var.vari_codigo = pal.vari_codigo  AND
			...
			ins.clie_codigo = ine.clie_codigo  AND
			ins.plde_codigo = ine.plde_codigo  AND
			ins.inpe_tipoin = ine.inpe_tipoin  AND
			ins.inpe_numero = ine.inpe_numero  AND
			ins.inpe_secuen = ine.inpe_secuen  AND
			(isnull(inpe_estado,0)) <> 5; 		  
...
ELSEIF sqlca.SQLCode = 100 THEN
	MessageBox("Atención", "Pallet No está en Condiciones de Devolver Certificado, Ingrese otro Código.", &
					Exclamation!, OK!)
	RETURN True
...
on w_gene_deta_archivo_devolucion.create
int iCurrent
call super::create
...
on w_gene_deta_archivo_devolucion.destroy
call super::destroy
destroy(this.cb_1)
end on
...
//IF Isnull(dw_1.Object.inpd_nrocer[il_fila]) OR dw_1.Object.inpd_nrocer[il_fila] = '' THEN
//	li_cont ++
//	ls_mensaje 			= ls_mensaje + "~nNúmero de Certificado"
//	ls_colu[li_cont]	= "inpd_nrocer"
//END IF
//
IF li_cont > 0 THEN
...
type pb_ultimo from w_mant_detalle`pb_ultimo within w_gene_deta_archivo_devolucion
end type

type pb_siguiente from w_mant_detalle`pb_siguiente within w_gene_deta_archivo_devolucion
end type

type pb_anterior from w_mant_detalle`pb_anterior within w_gene_deta_archivo_devolucion
end type

type pb_primero from w_mant_detalle`pb_primero within w_gene_deta_archivo_devolucion
end type

type pb_cancela from w_mant_detalle`pb_cancela within w_gene_deta_archivo_devolucion
integer x = 2711
integer y = 368
end type

type pb_acepta from w_mant_detalle`pb_acepta within w_gene_deta_archivo_devolucion
integer x = 2711
integer y = 156
end type

type pb_salir from w_mant_detalle`pb_salir within w_gene_deta_archivo_devolucion
integer x = 2711
integer y = 588
end type

type dw_1 from w_mant_detalle`dw_1 within w_gene_deta_archivo_devolucion
integer x = 73
integer width = 2455
integer height = 824
string dataobject = "dw_gene_archivo_devolucion"
end type
