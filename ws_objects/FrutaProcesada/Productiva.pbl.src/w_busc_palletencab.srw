$PBExportHeader$w_busc_palletencab.srw
forward
global type w_busc_palletencab from w_busqueda
end type
...
global w_busc_palletencab w_busc_palletencab
...
on w_busc_palletencab.create
int iCurrent
call super::create
end on

on w_busc_palletencab.destroy
call super::destroy
end on
...
ELSE
	...
	dw_1.Retrieve(Long(istr_busq.argum[1]), Long(istr_busq.argum[5]))
END IF
end event
...
//istr_busq.argum[8]	= String(dw_1.GetItemNumber(dw_1.GetRow(),"cont_codigo"))


CloseWithReturn(This,istr_busq)
end event

type pb_insertar from w_busqueda`pb_insertar within w_busc_palletencab
...
type dw_1 from w_busqueda`dw_1 within w_busc_palletencab
integer x = 37
integer y = 732
integer width = 2697
integer taborder = 60
...
type pb_salir from w_busqueda`pb_salir within w_busc_palletencab
integer x = 2834
integer y = 1368
...
type tab_1 from w_busqueda`tab_1 within w_busc_palletencab
integer x = 0
integer y = 96
integer width = 2693
end type
...
IF dw_1.Retrieve(Integer(istr_busq.argum[1]),Integer(istr_busq.argum[5])) > 0 THEN
	dw_1.SetFilter("")
	dw_1.Filter()

	IF is_TipoPallet <> "" THEN
		ls_Filtro	=	"paen_tipopa = " + is_TipoPallet
	END IF
	
	IF is_EstadoPallet <> "" THEN
		IF ls_Filtro <> "" THEN ls_Filtro += " AND "
		
		ls_Filtro	+=	"paen_estado = " + is_EstadoPallet
	END IF
	
	IF is_CondicPallet <> "" THEN
		IF ls_Filtro <> "" THEN ls_Filtro += " AND "
		
		ls_Filtro	+=	"paen_inspec = " + is_CondicPallet
		
		IF istr_busq.argum[6] <> "" THEN
			IF ls_Filtro <> "" THEN ls_Filtro += " OR "
		
		ls_Filtro	+=	"paen_inspec = " + istr_busq.argum[6]
		END IF
	...
	IF is_Planta <> "" THEN
		...
		ls_Filtro	+=	"plde_codigo = " + is_Planta
	END IF
