$PBExportHeader$uo_seleccion_planta_comun.sru
$PBExportComments$Objeto público para selección de Planta, Todas o Consolidada
forward
global type uo_seleccion_planta_comun from userobject
end type
type cbx_consolida from checkbox within uo_seleccion_planta_comun
end type
type cbx_todos from checkbox within uo_seleccion_planta_comun
end type
type dw_seleccion from datawindow within uo_seleccion_planta_comun
end type
end forward
...
global uo_seleccion_planta_comun uo_seleccion_planta_comun
...
public subroutine filtra (integer ai_cliente)
public subroutine limpiardatos ()
public subroutine bloquear (boolean ab_opcion)
end prototypes

event ue_cambio();this.dw_seleccion.Object.codigo[1]	=	This.Codigo
end event
...
public subroutine filtra (integer ai_cliente);dw_Seleccion.GetChild("codigo", idwc_Seleccion)

idwc_Seleccion.SetTransObject(sqlca)

IF idwc_Seleccion.Retrieve() = 0 THEN
	MessageBox("Atención", "No existen Planta para este Cliente seleccionado.")
...
on uo_seleccion_planta_comun.create
...
on uo_seleccion_planta_comun.destroy
destroy(this.cbx_consolida)
destroy(this.cbx_todos)
destroy(this.dw_seleccion)
end on

event constructor;dw_Seleccion.Object.Codigo.Dddw.Name			=	'dw_mues_plantadesp'
dw_Seleccion.Object.codigo.Dddw.DisplayColumn	=	'plde_nombre'
dw_Seleccion.Object.codigo.Dddw.DataColumn		=	'plde_codigo'
...
IF	idwc_Seleccion.Retrieve() = 0 THEN
	MessageBox("Atención", "No existen Plantas en tabla respectiva")
	
	SetNull(Codigo)
	SetNull(Nombre)
ELSE
	idwc_Seleccion.SetSort("plde_nombre A")
	idwc_Seleccion.Sort()
...
IF iuo_Planta.Existe(Integer(data), True, sqlca) THEN
	Codigo	=	iuo_Planta.Codigo
	Nombre	=	iuo_Planta.Nombre
ELSE
