$PBExportHeader$w_maed_palletencab_trans.srw
forward
global type w_maed_palletencab_trans from w_mant_encab_deta_csd
end type
type dw_histoencab from datawindow within w_maed_palletencab_trans
end type
type dw_histofruta from datawindow within w_maed_palletencab_trans
end type
end forward
...
global w_maed_palletencab_trans w_maed_palletencab_trans

type variables
Integer	ii_yaexiste, ii_estado, ii_ancestro, ii_Proceso
Long		il_NumProce
Boolean	ib_Existe

w_mant_deta_palletfruta_trans iw_mantencion
...
IF sqlca.SQLCode = -1 THEN
	F_errorbasedatos(sqlca,"Lectura tabla PalletEncab Transitorio")
	lb_retorno = True
ELSEIF ll_cantid > 0 THEN
	...
	IF sqlca.SQLCode = -1 THEN
		F_errorbasedatos(sqlca,"Lectura tabla PalletEncab Definitivo")
		lb_retorno = True
	ELSEIF ll_cantid > 0 THEN	
...
public function boolean buscaingreso ();Integer	li_Cliente, li_Planta, li_Cuenta, li_Cuenta2, li_Cuenta3 ,li_Cuenta4, &
			li_Cuenta5, li_Cuenta1, li_TipoEn, li_Origen
...
ELSEIF sqlca.SQLCode = 100 THEN
	...
	ELSEIF li_Cuenta > 0 THEN
		    ...
		    IF (li_Cuenta1 + li_Cuenta2 + li_Cuenta3 + li_Cuenta4 + li_Cuenta5) > 0 THEN
...
on w_maed_palletencab_trans.create
int iCurrent
call super::create
...
on w_maed_palletencab_trans.destroy
call super::destroy
...
OpenWithParm(w_busc_palletencab_trans, istr_busq)
...
type dw_1 from w_mant_encab_deta_csd`dw_1 within w_maed_palletencab_trans
integer x = 91
integer y = 1140
integer width = 2953
integer height = 752
...
type dw_2 from w_mant_encab_deta_csd`dw_2 within w_maed_palletencab_trans
integer x = 78
integer y = 56
integer width = 2985
integer height = 1060
...
type pb_nuevo from w_mant_encab_deta_csd`pb_nuevo within w_maed_palletencab_trans
integer y = 252
end type

type pb_eliminar from w_mant_encab_deta_csd`pb_eliminar within w_maed_palletencab_trans
integer y = 480
end type

type pb_grabar from w_mant_encab_deta_csd`pb_grabar within w_maed_palletencab_trans
integer y = 688
end type

type pb_imprimir from w_mant_encab_deta_csd`pb_imprimir within w_maed_palletencab_trans
integer y = 896
end type

type pb_salir from w_mant_encab_deta_csd`pb_salir within w_maed_palletencab_trans
integer y = 1156
end type

type pb_ins_det from w_mant_encab_deta_csd`pb_ins_det within w_maed_palletencab_trans
integer x = 3301
integer y = 1484
end type

type pb_eli_det from w_mant_encab_deta_csd`pb_eli_det within w_maed_palletencab_trans
integer x = 3301
integer y = 1660
end type

type pb_buscar from w_mant_encab_deta_csd`pb_buscar within w_maed_palletencab_trans
integer y = 76
end type
