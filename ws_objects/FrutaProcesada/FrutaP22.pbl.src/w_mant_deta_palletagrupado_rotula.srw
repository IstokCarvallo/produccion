$PBExportHeader$w_mant_deta_palletagrupado_rotula.srw
forward
global type w_mant_deta_palletagrupado_rotula from w_mant_detalle
end type
type dw_2 from datawindow within w_mant_deta_palletagrupado_rotula
end type
type dw_3 from datawindow within w_mant_deta_palletagrupado_rotula
end type
end forward
...
global w_mant_deta_palletagrupado_rotula w_mant_deta_palletagrupado_rotula
...
public function boolean noexistevariedad (integer ai_especie, integer ai_variedad);String	ls_nombre
Integer	li_cliente
Boolean	lb_retorna = True
Long		ll_codigo

	SELECT	vari_nombre INTO :ls_nombre
	FROM	dbo.variedades
	...
	ELSEIF sqlca.SQLCode = 100 THEN
		MessageBox("Atención", "Código de Variedad no ha sido creado, Ingrese otro Código.", &
		Exclamation!, OK!)
		lb_Retorna = True
	ELSE
		lb_retorna = False
...
on w_mant_deta_palletagrupado_rotula.create
int iCurrent
call super::create
...
on w_mant_deta_palletagrupado_rotula.destroy
call super::destroy
destroy(this.dw_2)
destroy(this.dw_3)
end on
...
IF istr_mant.argumento[11] = 'P' THEN
	dw_2.DataObject = "dw_mues_variedadrotulada"
	dw_3.DataObject = "dw_mues_variedadrotulada"		
	dw_1.DataObject = "dw_mues_palletfruta_rotula_origen_caja"	
ELSE
...
type pb_ultimo from w_mant_detalle`pb_ultimo within w_mant_deta_palletagrupado_rotula
boolean visible = false
end type

type pb_siguiente from w_mant_detalle`pb_siguiente within w_mant_deta_palletagrupado_rotula
boolean visible = false
end type

type pb_anterior from w_mant_detalle`pb_anterior within w_mant_deta_palletagrupado_rotula
boolean visible = false
end type

type pb_primero from w_mant_detalle`pb_primero within w_mant_deta_palletagrupado_rotula
boolean visible = false
end type

type pb_cancela from w_mant_detalle`pb_cancela within w_mant_deta_palletagrupado_rotula
boolean visible = false
integer x = 3936
integer y = 328
end type

type pb_acepta from w_mant_detalle`pb_acepta within w_mant_deta_palletagrupado_rotula
integer x = 3936
integer y = 116
end type

event pb_acepta::clicked;Long		ll_fila, ll_fila1, ll_fila2, ll_fil, ll_filaant
String		ls_variedad, ls_embalaje, ls_productor, ls_calibre, ls_huerto, ls_cuartel, ls_packing, ls_fecha, &
			ls_cliente, ls_folio,  ls_planta, ls_guia, ls_calibrerot, ls_categoria, ls_categoriareal, ls_Find
...
FOR ll_fila = 1 TO dw_1.RowCount()
	If IsNull(dw_1.Object.variedad[ll_fila]) OR dw_1.Object.variedad[ll_fila] = 0 Then
		MessageBox("Atención", "Falta el Ingreso de Nueva Variedad Rotulada.", Exclamation!, OK!)
		Return 1					
	End If	

	If IsNull(dw_1.Object.embalaje[ll_fila]) OR dw_1.Object.embalaje[ll_fila] = '' Then
		MessageBox("Atención", "Falta el Ingreso de Nuevo Embalaje Rotulado.", Exclamation!, OK!)
		Return 1			
	End If	
	
	If IsNull(dw_1.Object.productor[ll_fila]) OR dw_1.Object.productor[ll_fila] = 0 Then
		MessageBox("Atención", "Falta el Ingreso de Nuevo Productor Rotulado.",  Exclamation!, OK!)
		Return 1					
	End If	
	
	If IsNull(dw_1.Object.calibre[ll_fila]) OR dw_1.Object.calibre[ll_fila] = '' Then
		MessageBox("Atención", "Falta el Ingreso de Nuevo Calibre Rotulado.",  Exclamation!, OK!)
		Return 1			
	End If	

	If IsNull(dw_1.Object.packing[ll_fila]) OR dw_1.Object.packing[ll_fila] = 0 Then
		MessageBox("Atención", "Falta el Ingreso de Nuevo Packing Rotulado.", Exclamation!, OK!)
		Return 1					
	End If
	...
	If IsNull(dw_1.Object.fecha[ll_fila]) OR dw_1.Object.fecha[ll_fila] = Date('1900-01-01') Then
		MessageBox("Atención", "Falta el Ingreso de Nueva Fecha Rotulada.", Exclamation!, OK!)
		Return 1					
	End If	
	
	If IsNull(dw_1.Object.categoria[ll_fila]) OR dw_1.Object.categoria[ll_fila] = 0 Then
		MessageBox("Atención", "Falta el Ingreso de Nueva Categoria Rotulada.", Exclamation!, OK!)
		Return 1					
	End If
	
	If IsNull(dw_1.Object.categoriareal[ll_fila]) OR dw_1.Object.categoriareal[ll_fila] = 0 Then
		MessageBox("Atención", "Falta el Ingreso de Nueva Categoria Real.", Exclamation!, OK!)
		Return 1					
	End If
...
FOR ll_fila1 = 1 TO dw_1.RowCount()
		...
		ld_fecha				=	dw_1.Object.pafr_fecrot[ll_fila1]
		ls_folio				=	string(dw_1.Object.paen_numero[ll_fila1])
		ls_categoria			=	string(dw_1.Object.pafr_catrot[ll_fila1])
		ls_categoriareal	=	string(dw_1.Object.cate_codigo[ll_fila1])
		
		ls_Find = "clie_codigo = " + ls_cliente + " AND plde_codigo = " + ls_planta + " AND paen_numero = " + ls_folio + &
						" AND pafr_varrot = " + ls_variedad + " AND emba_codigo = '" + ls_embalaje + "'" + &
						" AND pafr_huert4 = " + ls_huerto + " AND pafr_cuart4 = " + ls_cuartel + &
						" AND pafr_prdrot = " + ls_productor + " AND pafr_catrot = " + ls_categoria + &
						" AND cate_codigo = " + ls_categoriareal + " AND pafr_calrot = '" + ls_calibre + "'" + &
						" AND pafr_rotpak = " + ls_packing + " AND Upper(String(pafr_fecrot,'dd/mm/yyyy')) Like'"+String(ld_fecha,'dd/mm/yyyy')+"%'"
		...
		If ll_fila > 0 Then
			DO WHILE ll_fila > 0
				...
				dw_2.Object.pafr_catrot[ll_fila] = dw_1.Object.categoria[ll_fila1]
				dw_2.Object.cate_codigo[ll_fila] = dw_1.Object.categoriareal[ll_fila1]
				dw_2.Object.pafr_calrot[ll_fila] = iuo_calibre.fomato_calibre(dw_1.Object.calibre[ll_fila1],sqlca)
				...
				dw_2.Object.pafr_fecrot[ll_fila] = dw_1.Object.fecha[ll_fila1]
				...
				ll_fila	= dw_2.Find(ls_Find, 1, dw_2.RowCount())
				If ll_filaant = ll_fila Then ll_fila = 0
			LOOP
		End If		 
NEXT		

MessageBox("Atención", "Cajas Aplicadas a Detalle.", Exclamation!, OK!)

end event

type pb_salir from w_mant_detalle`pb_salir within w_mant_deta_palletagrupado_rotula
integer x = 3936
integer y = 548
end type

type dw_1 from w_mant_detalle`dw_1 within w_mant_deta_palletagrupado_rotula
integer x = 27
integer y = 48
integer width = 3872
integer height = 1592
string dataobject = "dw_mues_palletfruta_rotula_origen_caja"
...
//			dw_1.Object.pafr_calrot[row] 	= iuo_calibre.fomato_calibre(data,sqlca)
			RETURN 1
		END IF	
	...
	CASE "variedad"	
		...
		IF noexistevariedad(dw_1.Object.espe_codigo[row],Integer(data)) THEN
			dw_1.SetItem(row, "variedad", Integer(ls_nula))
			Return 1
		END IF	
	...
	CASE "packing"	
		...
		IF noexistepacking(Long(data)) THEN
			dw_1.SetItem(row, "packing", long(ls_nula))
			Return 1
		END IF	
	...
	CASE "embalaje"	
		...
		IF existeembalaje(data,dw_1.Object.clie_codigo[row]) THEN
			dw_1.SetItem(row, "embalaje", ls_nula)
			Return 1
		END IF	
	...
	CASE "categoria"	
		...
		IF existecategoria(Integer(Data)) THEN
			dw_1.SetItem(row, "categoria", Integer(ls_Nula))
			RETURN 1
		END IF
	...
	CASE "categoriareal"	
		...
		IF existecategoria(Integer(Data)) THEN
			dw_1.SetItem(row, "categoriareal", Integer(ls_Nula))
			RETURN 1
		END IF				
