$PBExportHeader$w_gene_archivo_condicion_sag.srw
forward
global type w_gene_archivo_condicion_sag from window
end type
type dw_9 from datawindow within w_gene_archivo_condicion_sag
end type
type cbx_conex from checkbox within w_gene_archivo_condicion_sag
end type
type em_condicion from editmask within w_gene_archivo_condicion_sag
end type
type dw_8 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_7 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_3 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_4 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_5 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_6 from datawindow within w_gene_archivo_condicion_sag
end type
type cbx_var from checkbox within w_gene_archivo_condicion_sag
end type
type cbx_terceros from checkbox within w_gene_archivo_condicion_sag
end type
type st_7 from statictext within w_gene_archivo_condicion_sag
end type
type em_fecha from editmask within w_gene_archivo_condicion_sag
end type
type dw_11 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_10 from datawindow within w_gene_archivo_condicion_sag
end type
type st_4 from statictext within w_gene_archivo_condicion_sag
end type
type st_3 from statictext within w_gene_archivo_condicion_sag
end type
type sle_mensa from singlelineedit within w_gene_archivo_condicion_sag
end type
type st_5 from statictext within w_gene_archivo_condicion_sag
end type
type st_2 from statictext within w_gene_archivo_condicion_sag
end type
type pb_salir from picturebutton within w_gene_archivo_condicion_sag
end type
type pb_grabar from picturebutton within w_gene_archivo_condicion_sag
end type
type st_6 from statictext within w_gene_archivo_condicion_sag
end type
type st_1 from statictext within w_gene_archivo_condicion_sag
end type
type dw_2 from datawindow within w_gene_archivo_condicion_sag
end type
type dw_1 from datawindow within w_gene_archivo_condicion_sag
end type
end forward
...
string title = "ARCHIVO PLANO CONDICION"
...
global w_gene_archivo_condicion_sag w_gene_archivo_condicion_sag
...
Date		id_FechaAcceso
Time		it_HoraAcceso
Integer	ii_var, ii_estado, ii_tipoi = 1
...
public function boolean existefumigacion (long al_inspec)
end prototypes

event ue_guardar();Long			ll_Fila, ll_Filas, ll_FilaDet, ll_Numero, ll_numinpe, ll_fil
Integer		li_PldSag, li_cliente, li_planta, li_region, li_conexion
...
ll_Numero	= Long(em_condicion.text)
...
IF ii_estado <> 2 OR isnull(ii_estado) THEN 
	...
	ll_Filas		= dw_2.Retrieve(1,ll_Numero,Integer(istr_mant.argumento[1]), Integer(istr_mant.argumento[2]))
	
	IF ll_Filas = -1 THEN
		F_ErrorBaseDatos(sqlca,"Recuperación datos de Inspección")
	ELSEIF ll_Filas = 0 THEN
		MessageBox("Atención", "No hay información para Inspección Indicada.~r~rIngrese otro Número.", &
						Exclamation!, Ok!)
		pb_grabar.Enabled	= False
		em_condicion.SetFocus()
	ELSE
		...
		FOR ll_Fila = 1 TO ll_Filas
			...
			IF ls_Especie <> ls_EspeAnt THEN
				...
				ls_Registro	+= String(dw_2.Object.contpallet[1], '0000')
		...
		IF li_region <> 4 THEN
			ls_Archivo	= String(li_PldSag,'000') + String(ll_Numero,'00000') + ".FUM"
		ELSE	
			ls_Archivo	= String(li_PldSag,'0000') + String(ll_Numero,'00000') + ".FUM"
		END IF
		...
		ELSE
			...
			ll_numinpe = Long(em_condicion.Text)
	...
	em_condicion.SetFocus()
ELSE
	MessageBox("Atención", "la Inspección Ingresada se encuentra Cerrada.~r~rIngrese otro Número.", &
						Exclamation!, Ok!)
...
IF em_condicion.Text = "" THEN 
	MessageBox( "Advertencia", "Falta Número de Condición.", &
					StopSign!, Ok!)
	RETURN 1
ELSE
	ll_numinpe = Long(em_condicion.Text)
...
IF li_cliente = -1 THEN
	li_cliente = gi_Cliebase
END IF	
...
public function boolean existefumigacion (long al_inspec);Integer	li_codexp, li_planta
Date		ld_fecha
...
IF al_inspec <> 0 OR li_planta = 0 THEN
	...
	SELECT	Max(fumi_fecfum),fumi_estado
		INTO	:ld_fecha, :ii_estado
		FROM	dbo.fumigaenc
		WHERE	fumi_numsag	=	:al_inspec
		AND	cond_codigo =	:ii_tipoi		
		AND	clie_codigo	=	:li_codexp
		AND	plde_codigo =	:li_planta
		group by fumi_estado;
...
on w_gene_archivo_condicion_sag.create
...
on w_gene_archivo_condicion_sag.destroy
destroy(this.dw_9)
...
event modified;IF existefumigacion(Long(This.Text)) = False THEN
	This.SetFocus()
END IF
end event
...
long backcolor = 16711680
boolean enabled = false
string text = "Genera Archivo Plano Condición"
alignment alignment = center!
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
...
integer width = 247
integer height = 180
string dataobject = "dw_gene_archivo_condicion_pro"
end type
