$PBExportHeader$uo_recepcionbins.sru
forward
global type uo_recepcionbins from nonvisualobject
end type
end forward

global type uo_recepcionbins from nonvisualobject
end type
global uo_recepcionbins uo_recepcionbins

type variables
Integer	Cliente
Long		Planta, Numero
Datetime	Fecha
end variables

forward prototypes
public function boolean of_existe (integer ai_cliente, long al_planta, long al_numero, boolean ab_mensaje, transaction at_transaccion)
public function long of_maximo (integer ai_cliente, long al_planta, transaction at_transaccion)
public function boolean of_existebins (integer ai_cliente, long al_planta, long al_bins, transaction at_transaccion)
end prototypes

public function boolean of_existe (integer ai_cliente, long al_planta, long al_numero, boolean ab_mensaje, transaction at_transaccion);Boolean	lb_Retorno = True
 
SELECT	Distinct clie_codigo, plde_codigo, rece_numero, rece_fecrec
	INTO	:Cliente, :Planta, :Numero, :Fecha
	FROM	dbo.recepcion
	WHERE	clie_codigo = :ai_Cliente
	And		plde_codigo	=	:al_Planta
	And		rece_numero = :al_Numero
	USING	at_Transaccion;
	
IF at_Transaccion.SQLCode = -1 THEN
	F_ErrorBaseDatos(at_Transaccion, "Lectura de Tabla de Recepciones")
	lb_Retorno	=	False
ELSEIF at_Transaccion.SQLCode = 100 THEN
	lb_Retorno	=	False
 	IF ab_Mensaje THEN
		MessageBox("Atención", "Numero de Recepcion " + String(al_Numero) + &
					", o ha sido ingresado.~r~rIngrese o seleccione otro Numero.")	
	END IF
END IF

RETURN lb_Retorno
end function

public function long of_maximo (integer ai_cliente, long al_planta, transaction at_transaccion);Long	ll_Retorno = -1

SELECT	 IsNull(Max(rece_numero), 0) + 1
	INTO :ll_Retorno
	FROM	dbo.recepcion
	WHERE	clie_codigo = :ai_cliente
	And		plde_codigo	= :al_planta
	USING	at_Transaccion;
	
IF at_Transaccion.SQLCode = -1 THEN
	F_ErrorBaseDatos(at_Transaccion, "Lectura de Tabla de Recepciones")
	ll_Retorno	=	-1
ELSEIF at_Transaccion.SQLCode = 100 THEN
	ll_Retorno	=	-1
END IF

RETURN ll_Retorno
end function

public function boolean of_existebins (integer ai_cliente, long al_planta, long al_bins, transaction at_transaccion);Boolean	lb_Retorno = True
Long		ll_Cantidad
 
SELECT	Count(bins_numero)
	INTO :ll_Cantidad
	FROM	dbo.recepcion
	WHERE	clie_codigo = :ai_Cliente
	And		plde_codigo	= :al_Planta
	And		bins_numero = :al_bins
	USING	at_Transaccion;
	
IF at_Transaccion.SQLCode = -1 THEN
	F_ErrorBaseDatos(at_Transaccion, "Lectura de Tabla de Recepciones")
	lb_Retorno	=	False
ELSE
	If ll_Cantidad > 0 Then 
		lb_Retorno	=	False
		MessageBox("Atencion", "Numero de Bins se enuentra en otra recepcion.", StopSign!, Ok!)
	End If
END IF

RETURN lb_Retorno
end function

on uo_recepcionbins.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_recepcionbins.destroy
TriggerEvent( this, "destructor" )
call super::destroy
end on

